-- V10.16 · SASI RMB top-up RPC drift repair
-- The repository migration already supported the full RMB balance catalog,
-- but production DB had drifted back to the legacy 3-pack implementation.
-- This migration makes the desired state explicit again.
begin;
...
begin
  ...
  select x.amount_fen, x.amount_rmb
  into v_amount_fen, v_expected_rmb
  from (
    values
  ...
  if v_amount_fen is null
     and v_order.product_id ~ '^sasi-balance-custom-[0-9]{1,5}$'
  then
    v_expected_rmb :=
      substring(v_order.product_id from '([0-9]{1,5})$')::numeric;

    if v_expected_rmb between 10 and 10000
       and trunc(v_expected_rmb) = v_expected_rmb
    then
      v_amount_fen := (v_expected_rmb * 100)::bigint;
    end if;
  end if;

  if v_amount_fen is null then
    return jsonb_build_object('ok', false, 'error', 'invalid_sasi_product');
  end if;
  ...
  if v_order.amount_rmb is null
     or round(v_order.amount_rmb, 2) <> round(v_expected_rmb, 2)
  then
  ...
  if exists (
    ...
    return jsonb_build_object(
      'ok', true,
      'alreadyPaid', true,
      'amountFen', v_amount_fen
    );
  end if;

  insert into public.sasi_wallets(user_id)
  values (v_order.user_id)
  on conflict do nothing;

  select *
  into v_wallet
  from public.sasi_wallets
  where user_id = v_order.user_id
  for update;

  update public.sasi_wallets
  set available_points = available_points + v_amount_fen,
      updated_at = now()
  where user_id = v_order.user_id
  ...
  return jsonb_build_object(
    ...
    'alreadyPaid', false,
    'amountFen', v_amount_fen,
    'balanceFen', v_wallet.available_points,
    'reservedAmountFen', v_wallet.reserved_points
  );
end
$$;

revoke execute on function public.credit_sasi_topup(uuid)
from public, anon, authenticated;

grant execute on function public.credit_sasi_topup(uuid)
to service_role;
