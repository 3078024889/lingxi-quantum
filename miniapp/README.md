...
本目录采用“原生场域导航 + 原生树突精测 + 网页出版档案 + 原生微信权益”的双引擎架构。官网八类基础产品使用真实天文与历法演算；小程序八个独立场域与一个汇流层使用灵犀场树突知识网络（Lingxifield Dendritic Knowledge Network · LDKN），将情境选择转换为结构证据节点。两套引擎互为参照但不混算；付费后的完整小程序树突档案仍由网页出版层呈现和导出 PDF。
...
3. 微信后台服务器域名：
   ...
   - 9D 场域结构视频从 `https://lingxifield.cn/media/` 按需播放；确认该域名已在微信后台合法域名中。49 MB 高清源片只部署到官网，不得复制进小程序包。
   - 当前没有 WebSocket 或上传请求，不要为了填满表格而配置 socket/upload 域名。
