...
Page({
  ...
  refreshLanguage(lang){this.setData({lang,copy:copyFor('tools',lang),tools:listFor('tools',lang)})},
  ...
  open(event){const item=this.data.tools[event.currentTarget.dataset.index];if(!item)return;wx.navigateTo({url:`/pages/web/index?path=${encodeURIComponent(item.path)}`})},
