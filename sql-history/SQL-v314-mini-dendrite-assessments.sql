-- Lingxi v314 · Mini Program Copernican Dendrite assessments + ninth product
-- Run once in Supabase SQL Editor before publishing Mini Program v1.6.
...
-- Reinstall the explicit account-link RPC so native dendrite archives move
-- together with orders and all pre-v314 report tables.
...
declare
  v_identity_user_id uuid; v_target_openid text;
  v_source_manifest_until timestamptz; v_target_manifest_until timestamptz;
...
begin
  ...
  select p_target_user_id, product_id, expires_at from public.unlocks where user_id = p_source_user_id
  on conflict (user_id, product_id) do update set expires_at = case
    ...
    else greatest(public.unlocks.expires_at, excluded.expires_at) end;
