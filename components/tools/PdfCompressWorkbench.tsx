...
export default function PdfCompressWorkbench(){
  ...
  return <div className="space-y-4">
    <label onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();setFile(e.dataTransfer.files?.[0]||null)}} className="block cursor-pointer rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-7 text-center"><input type="file" accept="application/pdf" className="hidden" onChange={e=>setFile(e.target.files?.[0]||null)}/><b>Select PDF</b><p className="mt-1 text-sm text-slate-500">This mode compresses by rasterizing pages. It is best for scanned/image-heavy PDFs.</p></label>
