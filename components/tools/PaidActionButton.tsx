...
export default function PaidActionButton({toolId,quantity,metadata,onPaid,label}:{toolId:string;quantity:number;metadata?:Record<string,unknown>;onPaid:(quoteId:string)=>Promise<void>|void;label?:string}){
 ...
 async function complete(id:string){if(processing.current===id)return;processing.current=id;stop();setMsg(t(UI.paid));try{await onPaid(id)}finally{processing.current=null}}
 async function check(id:string){const r=await fetch(`/api/tools/pay/status?quoteId=${encodeURIComponent(id)}`,{cache:"no-store"});if(!r.ok)return false;const d=await r.json();if(!d.paid)return false;await complete(id);return true}
 useEffect(()=>{const h=(e:MessageEvent)=>{if(e.origin!==window.location.origin)return;const d=e.data as any;if(d?.type==="LINGXIFIELD_TOOL_PAYMENT_CONFIRMED"&&d.quoteId&&d.quoteId===quote?.id)void complete(d.quoteId)};window.addEventListener("message",h);return()=>{stop();window.removeEventListener("message",h)}},[quote?.id,lang]);
 ...
 function pay(){if(!quote)return;const w=window.open(`/tools/pay?quoteId=${encodeURIComponent(quote.id)}`,"lingxi_tool_pay","width=720,height=820");if(!w){setMsg(t(UI.blocked));return}stop();timer.current=setInterval(()=>void check(quote.id),2200);setMsg(t(UI.waiting))}
 const changed=quote&&Number(quote.quantity)!==Number(quantity);
 return <div>{!quote||changed?<button onClick={makeQuote} disabled={busy||quantity<=0} className="rounded-full bg-blue-600 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-40">{busy?t(UI.pricing):(label||t(UI.defaultLabel))}</button>:<div className="flex flex-wrap items-center gap-3"><div className="rounded-2xl bg-blue-50 px-4 py-2.5 text-sm text-blue-900">{t(UI.thisTime)} {quote.quantity} {quote.unit_name} · <b className="text-lg">¥{quote.amount_rmb}</b></div><button onClick={pay} className="rounded-full bg-blue-600 px-5 py-2.5 text-sm font-medium text-white">{t(UI.confirm)}</button><button onClick={()=>{stop();setQuote(null);setMsg("")}} className="rounded-full border border-slate-200 px-4 py-2.5 text-sm text-slate-600">{t(UI.recalc)}</button></div>}{msg&&<p className="mt-2 text-xs leading-5 text-slate-500">{msg}</p>}</div>
}
