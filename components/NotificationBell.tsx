...
export default function NotificationBell(){
 ...
 async function load(){try{const r=await fetch("/api/notifications",{cache:"no-store"});if(r.ok){const d=await r.json();setItems(Array.isArray(d.items)?d.items:[])}}catch{}}
 ...
 function copyFor(i:Item){if(i.kind==="announcement")return{title:t("announcementTitle"),body:releaseText(lang,i.announcementKey||"",{version:i.version||""})};if(i.kind==="payment")return{title:t("paymentArrived"),body:t("paymentBody",{amount:Number(i.amountRmb||0).toFixed(2)})};const s=i.status||"requested";return{title:t("refundUpdate"),body:t(s==="completed"?"refundCompleted":s==="approved"?"refundApproved":s==="rejected"?"refundRejected":"refundRequested")}}
 return <div className="lx11-notification-wrap" ref={ref}>
  <button className="lx11-icon-btn lx11-bell" aria-label={t("notifications")} aria-expanded={open} onClick={toggle}><LingxiMiniIcon name="sparkles" size="tiny"/>{unread>0&&<span className="lx11-notification-count">{unread>9?"9+":unread}</span>}</button>
  {open&&<div className="lx11-notification-panel lx-notification-v201">
   <header><b>{t("notifications")}</b>{unread>0&&<span>{unread}</span>}</header>
   {items.length===0?<p className="lx11-notification-empty"><span aria-hidden="true">✓</span>{t("allCaughtUp")}</p>:<div className="lx11-notification-list">
    {items.slice(0,12).map(i=>{const c=copyFor(i);const body=<><strong>{c.title}</strong><p>{c.body}</p><small>{new Date(i.createdAt).toLocaleString(lang==="zh"?"zh-CN":lang)}</small></>;return i.kind==="announcement"?<article key={i.id} className="lx-notification-announcement">{body}</article>:i.href?<Link key={i.id} href={i.href} onClick={()=>setOpen(false)}>{body}</Link>:<article key={i.id}>{body}</article>})}
   </div>}
  </div>}
 </div>
}
