...
export default function SubconsciousExplorer({ initialGate = "health" }: { initialGate?: string }) {
  ...
  return <div className="subconscious-explorer">
    <nav className="subconscious-gates" aria-label="六道观察入口">{gates.map((item, index) => <button key={item.id} aria-pressed={gate.id === item.id} onClick={() => setSelected(item.id)}>
      <div className="subconscious-gate-art"><img src={`/images/subconscious/v2/${item.id}.webp`} alt="" loading="lazy" width={768} height={512}/></div>
    ...
    <section className="subconscious-reading" aria-label={`${gate.title} · 观察与练习`}>
      <header><div className="subconscious-reading-art"><img src={`/images/subconscious/v2/${gate.id}.webp`} alt="" width={768} height={512}/></div><div><small>REFLECTION · FREE</small><h2><Bi zh={gate.title} en={gate.titleEn} /></h2><p><Bi zh={gate.line} en={gate.lineEn} /></p></div></header>
