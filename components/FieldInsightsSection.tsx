...
const INSIGHTS: FieldInsight[] = [
  ...
  {href:"/mirror",no:"07",glyph:"✦",zh:"灵犀量子生命镜像",en:"Lingxi Quantum Life Mirror",leadZh:"在三重镜像中，看见此刻的自己",leadEn:"See yourself now through three mirrors",bodyZh:[
    ...
    "You are not looking to know the answer ahead of time, but to see one more layer of yourself and make room for a clearer choice before life continues to unfold."],timeZh:"补充更具体的降生时刻，可展开更多与时间位置相关的结构层次，让三重镜像之间更细微的连接被进一步照见。",timeEn:"A precise birth time unfolds subtler time-and-position layers, revealing finer connections among the three mirrors.",ctaZh:"生成我的三重生命镜像",ctaEn:"Generate My Three Life Mirrors"},
  {href:"/qian",no:"08",glyph:"✦",zh:"灵犀生命灵签 · 意识坐标读取",en:"Lingxi Life Oracle · Consciousness Coordinate Reading",leadZh:"看见此刻与你发生回应的三枚灵签",leadEn:"Meet the three signs responding to you now",bodyZh:[
    "六十四枚原创签象中，会有三枚在此刻与你形成更清晰的连接。灵犀场以你的出生信息作为时间入口，将这组个人坐标映射进 64 枚签象库，并从三层结构中展开读取。",
    "源流签：照见你长期携带的背景与底层倾向。灵魂签：照见此刻最值得被看见的核心模式。行者签：照见你正在如何把内在选择带入现实。",
    "真正有意义的，不是单独看某一枚签，而是看三枚签象放在一起之后，彼此之间形成了怎样的呼应。",
    "这些呈现，最终仍会回到你的真实经历中被确认——你正在经历什么、什么反复出现、什么正在发生变化，都会成为理解这组三签的参照。静心片刻，让三枚灵签自然显现。"],bodyEn:[
    "Among sixty-four original signs, three form a clearer connection with you now. Lingxi Field uses your birth information as an entrance through time, maps your coordinates into the sign library, and reads them through three layers.",
    ...
    "Meaning comes not from any one sign alone, but from the resonance formed when all three signs are placed together.",
  ...
  {href:"/archetype",no:"09",glyph:"◎",zh:"生命原型",en:"Life Archetype",leadZh:"让八重场域，在此刻汇成一幅生命原型",leadEn:"Let eight fields converge into a life archetype for this moment",bodyZh:[
    "生命原型不是另一份孤立答案。它位于灵犀场「场域精测」的汇流层。",
    "生命图谱留下长期结构，关系共振留下连接结构，生命灵签与生命镜像留下象征映照，生命韧性记录压力与恢复，桃花磁场记录吸引与靠近，财富创造地图记录价值创造路径，今日潮汐记录当下状态与节律。",
    "当这些已经形成的场域节点，与此刻的真实选择重新进入同一张知识网络，原本分散在不同维度中的信号，会开始彼此连接。",
    "系统读取的不是八个结果的简单叠加，而是哪些节点正在共同增强，哪些结构正在彼此牵引，哪些力量长期存在，哪些信号最近才变得清晰，以及哪些结构之间正在形成真实张力。",
    "这些信号在节点激活后彼此相连，并经过结构增强、抑制与跨域校准，形成此刻的生命原型。它不是随机抽取，不是固定人格，更不是未来判定。",
    "它呈现的是：八重场域在这一刻，共同形成的生命结构。"],bodyEn:[
    "Life Archetype is not another isolated answer. It is the convergence layer of Lingxi Field’s Field Insights.",
    "Life Blueprint carries long-term structure; Relationship Resonance carries connection patterns; Life Oracle and Life Mirror carry symbolic reflection; Resilience records pressure and recovery; Romance records attraction and approach; Wealth maps value creation; Today’s Tide records present state and rhythm.",
    "When these established field nodes re-enter one knowledge network with your present choices, signals once distributed across different dimensions begin to connect.",
    "The engine does not simply add eight results. It reads which nodes reinforce one another, which structures exert mutual pull, which forces persist over time, which signals have only recently become clear, and where real tension is forming.",
    "Through node activation, linked propagation, structural inhibition, and cross-field calibration, these signals form your current Life Archetype. It is not random, a fixed personality, or a judgment about the future.",
    "It reveals the life structure formed by eight fields together, in this moment."],ctaZh:"展开我的生命原型",ctaEn:"Unfold My Life Archetype"},
];
...
export default function FieldInsightsSection(){
  return <section id="field-insights" className="relative overflow-hidden border-t border-white/5 px-5 py-24 sm:px-6 sm:py-36">
    <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_15%_15%,rgba(98,214,203,.09),transparent_30%),radial-gradient(circle_at_85%_70%,rgba(193,145,255,.08),transparent_34%)]"/>
    <div className="relative mx-auto max-w-6xl">
      <header className="mx-auto max-w-3xl text-center">
        <p className="font-display text-sm uppercase tracking-widest2 text-lattice"><Bi zh="场域精测" en="Field Insights"/></p>
        <h2 className="mt-6 font-display text-4xl font-light text-bone sm:text-5xl"><Bi zh="十个入口，同一场生命的回声" en="Ten Entrances into One Living Field"/></h2>
        <p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-bone-dim"><Bi zh="选择此刻与你发生回应的入口。这里没有替你定义的结论，只有可以带回真实经历中、由你亲自确认的多维映照。" en="Choose the entrance that responds to you now. Nothing here defines you; each reflection returns to your lived experience for you to recognize and confirm."/></p>
        <div className="mt-7 border border-lattice/25 bg-lattice/[.04] p-5 text-left text-sm leading-7 text-bone-dim"><p className="font-display text-lattice"><Bi zh="双引擎 · 两种观察路径" en="Dual Engines · Two Paths of Observation"/></p><p className="mt-2"><Bi zh="官网从天文、历法与时间坐标进入，展开相对稳定的生命结构；微信小程序从真实选择、行为情境与当下状态进入，通过灵犀场树突知识网络持续形成个人结构记录。两者独立计算，可以彼此映照，不混算，也不预测未来。" en="The website enters through astronomical, calendrical and temporal coordinates to unfold relatively stable life structure. The WeChat Mini Program enters through lived choices, behavioural contexts and present state, using the Lingxifield Dendritic Knowledge Network to form personal structural records. The two paths are calculated independently and may cross-reflect without being mixed or predicting the future."/></p></div>
      </header>
      <div className="mt-16 space-y-7">{INSIGHTS.map((item)=><article key={item.href} className="lx-glass group relative overflow-hidden rounded-sm border border-white/10 p-6 transition duration-500 hover:border-lattice/35 sm:p-9 lg:p-12"><div className="relative grid gap-8 lg:grid-cols-[minmax(220px,.72fr)_1.8fr] lg:gap-14"><div><p className="text-xs uppercase tracking-[.32em] text-bone-mute">FIELD {item.no}</p><div className="mt-5 text-3xl" aria-hidden>{item.glyph}</div><h3 className="mt-4 font-display text-2xl font-light text-bone sm:text-3xl"><Bi zh={item.zh} en={item.en}/></h3><p className="mt-3 font-display text-base leading-7 text-lattice"><Bi zh={item.leadZh} en={item.leadEn}/></p></div><div><div className="space-y-4 text-[15px] leading-8 text-bone-dim sm:text-base">{item.bodyZh.map((zh,i)=><p key={i}><Bi zh={zh} en={item.bodyEn[i]}/></p>)}</div><Link href={item.href} className="mt-8 inline-flex items-center gap-3 border border-lattice/40 bg-lattice/5 px-6 py-3 font-display text-sm tracking-wider text-lattice transition hover:border-lattice hover:bg-lattice hover:text-void-deep"><span aria-hidden>✨</span><Bi zh={item.ctaZh} en={item.ctaEn}/><span aria-hidden>→</span></Link>{item.closingZh&&<p className="mt-6 text-sm italic leading-7 text-bone-soft"><Bi zh={item.closingZh} en={item.closingEn||""}/></p>}</div></div></article>)}</div>
    </div>
  </section>;
}
