...
export default function SasiLegalPage() {
  return <><Nav /><main className="px-6 pb-28 pt-28"><article className="mx-auto max-w-4xl"><p className="text-xs uppercase tracking-[.22em] text-[var(--lx-ink)]">LingxiField SASI</p><h1 className="mt-4 font-display text-4xl font-light text-[var(--lx-ink)] sm:text-6xl"><Bi zh="法律与规则" en="Legal & Rules" /></h1><p className="mt-6 max-w-2xl leading-8 text-[var(--lx-muted)]"><Bi zh="以下是 SASI 第一阶段的双语平台规则摘要。充值、用户 Skill 交易和真实模型调用正式开放前，将补充经营主体、联系渠道、数据位置及各 Provider 的最终条款。" en="This bilingual summary governs SASI phase one. Before payments, user Skill trading or live model calls open, the final operator details, contact channel, data location and provider-specific terms will be added." /></p><div className="mt-14 space-y-5">{sections.map((section) => <section key={section.titleEn} className="rounded-sm border border-[var(--lx-line)] bg-[var(--lx-panel)] p-7"><h2 className="font-display text-2xl text-[var(--lx-ink)]"><Bi zh={section.titleZh} en={section.titleEn} /></h2><p className="mt-4 leading-8 text-[var(--lx-muted)]"><Bi zh={section.bodyZh} en={section.bodyEn} /></p></section>)}</div><p className="mt-10 text-xs leading-6 text-[var(--lx-faint)]"><Bi zh="版本：SASI Phase 1 · 2026-09-07。本页为产品规则摘要，不替代针对具体经营和数据处理活动的专业法律审查。" en="Version: SASI Phase 1 · 2026-09-07. This page is a product-rule summary and does not replace professional legal review for the actual operation and data-processing activities." /></p></article></main><Footer /></>;
}
