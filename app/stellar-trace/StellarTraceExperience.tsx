...
export default function StellarTraceExperience({ unlocked = false, initialDraft }: { unlocked?: boolean; initialDraft?: Partial<StellarTraceDraft> | null }) {
  ...
  const summary = result ? `灵犀场星迹 · 万里寻踪\n主核验方位：${result.priority.primaryDirectionZh} ${result.priority.primaryBearing}°\n推演状态：${result.priority.statusZh}\n核验扇区：${sectorLabel(result.priority.primarySector)}\n圆周集中度：R=${result.direction.resultantLength.toFixed(3)}${result.distance.rangeKm ? `\n现实核验距离带：${result.distance.rangeKm[0]}—${result.distance.rangeKm[1]} km` : ""}${result.candidateCenter ? `\n候选投影中心：${result.candidateCenter.lat}, ${result.candidateCenter.lon}\n核验半径：${result.candidateCenter.radiusKm} km` : ""}\n${result.priority.basisZh}\n\n${result.priority.verificationZh.join("\n")}\n\n${result.modelBoundaryZh}\n${result.safetyBoundaryZh}` : "";
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_14%_8%,rgba(121,96,186,.34),transparent_30%),radial-gradient(circle_at_86%_22%,rgba(40,155,168,.25),transparent_34%),#07112d] px-4 py-14 text-bone sm:px-6">
    ...
    {result && <section id="stellar-result" className="mx-auto mt-16 max-w-[794px] scroll-mt-8"><StellarTraceVisualization result={result} mode="live"/><div ref={resultRef} className="mt-7 space-y-7">
      <PublicationPage index={1} total={2} eyebrow="EVIDENCE CONVERGENCE · VERIFICATION PRIORITY" title="诸证合度" art={art(result.artIndexes[0])} layout="cover"><PublicationLabel>STELLAR TRACE CONVERGENCE INSTRUMENT</PublicationLabel><p className="mt-4 font-display text-[34px] leading-tight text-[#557f79]">主核验方位 · {result.priority.primaryDirectionZh} {result.priority.primaryBearing}°</p><PublicationCopy>圆周集中度 R = {result.direction.resultantLength.toFixed(3)}，当前为“{result.priority.statusZh}”。核验扇区：{sectorLabel(result.priority.primarySector)}。{result.priority.basisZh}</PublicationCopy><StellarTraceVisualization result={result} mode="print"/></PublicationPage>
      <PublicationPage index={2} total={2} eyebrow="NINE-FIELD EVIDENCE · REALITY VERIFICATION" title="九域据链 · 现实核验" art={art(result.artIndexes[1])} layout="full"><PublicationLabel>先据实，后参象；象与实逆，则舍象从实</PublicationLabel><div className="mt-4 grid grid-cols-3 gap-2">{result.snapshots.map((snapshot) => <div key={snapshot.epoch} className="border border-[#4c4966]/15 bg-white/25 p-3"><p className="text-[13px] font-semibold text-[#557f79]">{snapshot.labelZh}</p><p className="mt-1 text-[11px] text-[#565162]">{new Date(snapshot.observedAt).toLocaleString("zh-CN", { hour12: false })}</p><p className="mt-1 text-[9px] text-[#88818f]">JD {snapshot.julianDay}</p></div>)}</div><div className="mt-5 space-y-3">{result.evidence.map((item) => <div key={item.id} className="border-l-2 border-[#a78e63]/55 pl-3"><p className="text-[14px] font-semibold">{item.labelZh} · {item.bearing}°</p><p className="mt-1 text-[13px] leading-6 text-[#565162]">{item.projectionBasisZh}</p></div>)}</div><div className="mt-5 border border-[#557f79]/25 bg-white/35 p-4"><p className="font-display text-xl text-[#557f79]">现实核验次序</p>{result.priority.verificationZh.map((line,index)=><p key={line} className="mt-2 text-[13px] leading-6">{index+1}. {line}</p>)}{result.priority.conflictsZh.map(line=><p key={line} className="mt-2 text-[12px] leading-6 text-[#7b5f55]">冲突说明：{line}</p>)}</div><div className="mt-4 border border-[#8b6759]/25 bg-[#fff8f1]/55 p-4"><p className="font-display text-lg text-[#6f5147]">{result.distance.rangeKm ? "现实距离带与候选投影" : "距离与坐标止于证界"}</p><p className="mt-2 text-[13px] leading-6">{result.distance.explanationZh}</p>{result.candidateCenter&&<p className="mt-2 text-[13px] leading-6">候选投影中心：{result.candidateCenter.lat}, {result.candidateCenter.lon} · 核验半径 {result.candidateCenter.radiusKm} km。此点为扇区与距离带的几何中心，不是定位事实。</p>}</div><p className="mt-4 border-t border-[#4c4966]/15 pt-3 text-[12px] leading-6 text-[#696473]">{result.modelBoundaryZh}<br/>{result.safetyBoundaryZh}</p></PublicationPage>
