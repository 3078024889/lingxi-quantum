...
export default function StellarTraceExperience({ unlocked = false, initialDraft }: { unlocked?: boolean; initialDraft?: Partial<StellarTraceDraft> | null }) {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_14%_8%,rgba(121,96,186,.34),transparent_30%),radial-gradient(circle_at_86%_22%,rgba(40,155,168,.25),transparent_34%),#07112d] px-4 py-14 text-bone sm:px-6">
    ...
    <form onSubmit={submit} className="mx-auto mt-12 max-w-5xl border border-white/10 bg-white/[.045] p-5 backdrop-blur-xl sm:p-8">
      ...
      {!unlocked && <label className="mt-5 flex items-start gap-3 text-xs leading-6 text-bone-dim"><input required type="checkbox" checked={boundaryConsent} onChange={(e) => setBoundaryConsent(e.target.checked)} className="mt-1"/><span>我已理解：本次付费会给出主核验方向、三层候选距离区与现实核验次序；候选区是移动情景，不把天文角度伪装成定位事实。</span></label>}
    ...
    {result && <section id="stellar-result" className="mx-auto mt-16 max-w-[794px] scroll-mt-8"><StellarTraceVisualization result={result} mode="live"/><div ref={resultRef} className="mt-7 space-y-7">
      ...
      <PublicationPage index={2} total={2} eyebrow="NINE-FIELD EVIDENCE · REALITY VERIFICATION" title="三层候选区 · 现实核验" art={art(result.artIndexes[1])} layout="full"><PublicationLabel>先据实，后参象；由近及远，逐层排除</PublicationLabel><p className="mt-3 text-[12px] leading-6 text-[#696473]">{result.distance.explanationZh}</p><div className="mt-3 grid gap-2">{result.candidateZones.map((zone)=><div key={zone.rank} className="border border-[#8b6759]/20 bg-white/55 p-3"><p className="text-[14px] font-semibold text-[#557f79]">{zone.labelZh} · {zone.distanceRangeKm[0]}—{zone.distanceRangeKm[1]} km</p>{zone.center&&<p className="mt-1 text-[12px]">候选中心 {zone.center.lat}, {zone.center.lon} · 核验半径 {zone.radiusKm} km</p>}<p className="mt-1 text-[12px] leading-5 text-[#565162]">{zone.basisZh}</p><p className="mt-1 text-[12px] leading-5">核验：{zone.verificationZh}</p></div>)}</div><div className="mt-4 border border-[#557f79]/25 bg-white/35 p-4"><p className="font-display text-lg text-[#557f79]">现实核验次序</p>{result.priority.verificationZh.map((line,index)=><p key={line} className="mt-2 text-[12px] leading-5">{index+1}. {line}</p>)}{result.priority.conflictsZh.map(line=><p key={line} className="mt-2 text-[11px] leading-5 text-[#7b5f55]">冲突说明：{line}</p>)}</div><p className="mt-4 border-t border-[#4c4966]/15 pt-3 text-[11px] leading-5 text-[#696473]">{result.modelBoundaryZh}<br/>{result.safetyBoundaryZh}</p></PublicationPage>
