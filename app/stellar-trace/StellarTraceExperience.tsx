...
function NonPersonTraceForm({kind,unlocked,initialDraft}:{kind:"object"|"animal";unlocked:boolean;initialDraft?:Partial<StellarTraceDraft>|null}){
  const[name,setName]=useState("");const[subtype,setSubtype]=useState(kind==="animal"?"cat":"wallet");const[lastSeenAt,setLastSeenAt]=useState("");const[place,setPlace]=useState("");const[lat,setLat]=useState("");const[lon,setLon]=useState("");const[context,setContext]=useState("");const[consent,setConsent]=useState(false);const[loading,setLoading]=useState(false);const[error,setError]=useState("");const[result,setResult]=useState<TargetTraceResult|null>(null);const reportRef=useRef<HTMLDivElement>(null);
  useEffect(()=>setSubtype(kind==="animal"?"cat":"wallet"),[kind]);
  ...
  async function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();setError("");if(!name||!lastSeenAt||!place||!lat||!lon)return setError("请完整填写目标、最后确认时间、地点与地图坐标。");if(!consent)return setError("请先确认现实安全边界。");const payload={targetKind:kind,targetName:name,queryTime:new Date().toISOString(),lastSeenAt,lastKnownPlace:place,lastKnownLat:Number(lat),lastKnownLon:Number(lon),context,...(kind==="animal"?{animalKind:subtype,microchipped:null,indoorOutdoor:"unknown",temperament:"unknown"}:{objectKind:subtype,container:null,lastHandledBy:null,likelyTransport:"unknown"})};if(!unlocked){window.name=JSON.stringify({kind:"lingxifield-stellar-target-v340",draft:payload});window.location.href="/checkout?productId=stellar-trace&redirect=%2Fstellar-trace&intake=complete";return;}setLoading(true);try{const response=await fetch("/api/stellar-trace/target",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const data=await response.json();if(!response.ok)throw new Error(data.error||"联合推演暂未形成");setResult(data);}catch(reason){setError(reason instanceof Error?reason.message:"联合推演暂未形成");}finally{setLoading(false);}}
  async function download(){if(!reportRef.current)return;const{exportPublicationPagesPdf}=await import("@/lib/pdf-export");await exportPublicationPagesPdf({containerRef:reportRef.current,fileName:`灵犀场星迹-${kind==="animal"?"寻动物":"寻物"}-${name}.pdf`});}
  const animalOptions=[["cat","猫"],["dog","狗"],["bird","鸟"],["livestock","牲畜"],["other","其他动物"]];const objectOptions=[["wallet","钱包"],["phone","手机"],["keys","钥匙"],["document","证件/文件"],["jewelry","首饰"],["bag","包袋"],["vehicle","车辆"],["other","其他物品"]];
  return <><form onSubmit={submit} className="mx-auto mt-12 max-w-5xl border border-white/10 bg-white/[.045] p-5 backdrop-blur-xl sm:p-8"><p className="text-xs tracking-[.26em] text-amber">{kind==="animal"?"ANIMAL TRACE · 寻动物":"OBJECT TRACE · 寻物"}</p><h2 className="mt-2 font-display text-3xl">最后可证之处</h2><p className="mt-3 text-sm leading-7 text-bone-dim">原典象、现实接触链与行为搜索层彼此分开。证据不足即停止，不以目标名称、已知方向或习性反造古法方位。</p><div className="mt-6 grid gap-5 sm:grid-cols-2"><label className="text-xs text-bone-dim">{kind==="animal"?"动物名字":"物品名称"} *<input required value={name} onChange={event=>setName(event.target.value)} className={inputClass}/></label><label className="text-xs text-bone-dim">类别 *<select value={subtype} onChange={event=>setSubtype(event.target.value)} className={inputClass}>{(kind==="animal"?animalOptions:objectOptions).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label className="text-xs text-bone-dim sm:col-span-2">最后确认时间 *<input required type="datetime-local" value={lastSeenAt} onChange={event=>setLastSeenAt(event.target.value)} className={inputClass}/></label><label className="text-xs text-bone-dim sm:col-span-2">最后确认位置 *<input required value={place} onChange={event=>setPlace(event.target.value)} className={inputClass}/></label><PreciseMapPicker lat={lat} lon={lon} place={place} onConfirm={({lat:nextLat,lon:nextLon})=>{setLat(nextLat.toFixed(7));setLon(nextLon.toFixed(7));}}/><label className="text-xs text-bone-dim sm:col-span-2">现实线索<textarea value={context} onChange={event=>setContext(event.target.value)} rows={4} className={`${inputClass} resize-none`}/></label></div><label className="mt-6 flex gap-3 text-xs leading-6 text-bone-dim"><input type="checkbox" checked={consent} onChange={event=>setConsent(event.target.checked)} className="mt-1"/><span>我确认资料来源真实，并理解本结果不能替代监控、设备定位、警方、救援、芯片平台或现实接触链。</span></label><button disabled={loading} className="mt-7 w-full border border-lattice/60 bg-lattice/10 px-6 py-4 text-sm tracking-[.18em] text-lattice disabled:opacity-40">{loading?"正在分层推演…":unlocked?"展开联合推演":"确认边界并开启 · ¥688"}</button>{error&&<p className="mt-4 text-sm text-rose-300">{error}</p>}</form>{result&&<section className="mx-auto mt-12 max-w-[794px]"><div ref={reportRef} className="space-y-7"><PublicationPage index={1} total={2} eyebrow="ANCIENT EVIDENCE · NO FABRICATION" title={`${kind==="animal"?"寻动物":"寻物"} · 原典证界`} art={art(7)} layout="cover"><PublicationLabel>{name} · 已成 {result.ancient.fused.usedSystems.length} / 4</PublicationLabel><PublicationCopy>{result.ancient.fused.rationaleZh}</PublicationCopy>{result.ancient.results.map(item=><p key={item.system} className="mt-2 text-[12px] leading-5">{item.system.toUpperCase()} · {item.status} · {item.direction?.labelZh??"未成向"}</p>)}</PublicationPage><PublicationPage index={2} total={2} eyebrow="REALITY SEARCH · PRACTICAL PRIORITY" title="现实搜索次序" art={art(19)} layout="full">{result.realityHints.searchPriorityZh.map((line,index)=><p key={line} className="mt-3 text-[13px] leading-6">{index+1}. {line}</p>)}<PublicationCopy muted>{result.notesZh.join(" ")}</PublicationCopy></PublicationPage></div><button onClick={download} className="mt-6 w-full border border-lattice/45 py-3 text-xs tracking-widest2 text-lattice">下载 2 页联合推演 PDF</button></section>}</>;
}
...
export default function StellarTraceExperience({ unlocked = false, initialDraft }: { unlocked?: boolean; initialDraft?: Partial<StellarTraceDraft> | null }) {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_14%_8%,rgba(121,96,186,.34),transparent_30%),radial-gradient(circle_at_86%_22%,rgba(40,155,168,.25),transparent_34%),#07112d] px-4 py-14 text-bone sm:px-6">
    ...
    {targetKind!=="person"?<NonPersonTraceForm kind={targetKind} unlocked={unlocked} initialDraft={initialDraft}/>:<form onSubmit={submit} className="mx-auto mt-12 max-w-5xl border border-white/10 bg-white/[.045] p-5 backdrop-blur-xl sm:p-8">
      ...
      {!unlocked && <div className="mt-6 border border-amber/35 bg-amber/[.06] p-5"><p className="text-xs tracking-[.2em] text-amber">支付前结果边界</p><p className="mt-3 text-sm leading-7 text-bone-soft">¥688 交付九域天文事实、原典四证覆盖、独立圆周合度、现实方向核验与完整研究档案。只有真实起局证据通过合度门时才生成候选方位；不以用户填写方向反推答案，也不把古法里数伪换算为公里或坐标。</p><p className="mt-2 text-xs leading-6 text-bone-dim">技术故障、重复支付或支付后无法开启，依《退款政策》处理。</p></div>}
    ...
    {targetKind==="person"&&result && <section id="stellar-result" className="mx-auto mt-16 max-w-[794px] scroll-mt-8"><StellarTraceVisualization result={result} mode="live"/><div ref={resultRef} className="mt-7 space-y-7">
      <PublicationPage index={1} total={2} eyebrow="ANCIENT EVIDENCE · INDEPENDENT CONVERGENCE" title="原典四证 · 独立合度" art={art(result.artIndexes[0])} layout="cover"><PublicationLabel>STELLAR TRACE CONVERGENCE INSTRUMENT</PublicationLabel><p className="mt-4 font-display text-[34px] leading-tight text-[#557f79]">{result.priority.available ? `候选方位 · ${result.priority.primaryDirectionZh} ${result.priority.primaryBearing}°` : `原典覆盖 · ${result.ancient.fused.usedSystems.length} / 4`}</p><PublicationCopy>{result.priority.available ? `候选扇区：${sectorLabel(result.priority.primarySector)}。` : "真实起局证据尚未通过合度门。"}{result.priority.basisZh}</PublicationCopy><StellarTraceVisualization result={result} mode="print"/></PublicationPage>
      <PublicationPage index={2} total={2} eyebrow="NINE-FIELD FACTS · REALITY VALIDATION" title="九域事实 · 现实核验" art={art(result.artIndexes[1])} layout="full"><PublicationLabel>原典先独立成向，现实线索只作事后核验</PublicationLabel><p className="mt-3 text-[12px] leading-6 text-[#696473]">{result.distance.explanationZh}</p><div className="mt-3 border border-[#8b6759]/20 bg-white/55 p-4"><p className="text-[14px] font-semibold text-[#557f79]">现实方向核验</p><p className="mt-2 text-[12px] leading-5 text-[#565162]">{result.ancient.realityValidation.noteZh}</p></div><div className="mt-4 border border-[#557f79]/25 bg-white/35 p-4"><p className="font-display text-lg text-[#557f79]">现实核验次序</p>{result.priority.verificationZh.map((line,index)=><p key={line} className="mt-2 text-[12px] leading-5">{index+1}. {line}</p>)}{result.priority.conflictsZh.map(line=><p key={line} className="mt-2 text-[11px] leading-5 text-[#7b5f55]">证据说明：{line}</p>)}</div><p className="mt-4 border-t border-[#4c4966]/15 pt-3 text-[11px] leading-5 text-[#696473]">{result.modelBoundaryZh}<br/>{result.safetyBoundaryZh}</p></PublicationPage>
