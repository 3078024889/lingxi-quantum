...
function NonPersonTraceForm({kind,unlocked,initialDraft}:{kind:"object"|"animal";unlocked:boolean;initialDraft?:Partial<StellarTraceDraft>|null}){
  ...
  return <>
    <form onSubmit={submit} className="mx-auto mt-12 max-w-5xl border border-white/10 bg-white/[.045] p-5 backdrop-blur-xl sm:p-8">
      ...
      {kind==="animal"&&<p className="mt-4 text-xs leading-6 text-bone-mute">犬与明确牲畜可进入奇门六畜证；猫、泛称鸟类及其他动物若无真实六爻起卦，报告会保留证界并强化现实搜索次序。</p>}
    ...
    {result&&<section className="mx-auto mt-12 max-w-[794px]"><div ref={reportRef} className="space-y-7">
      <PublicationPage index={1} total={3} eyebrow="ANCIENT EVIDENCE · CONVERGENCE" title={`${traceName} · 诸证合度`} art={art(7)} layout="cover"><PublicationLabel>{name} · 原典覆盖 {formed.length} / 4</PublicationLabel><p className="mt-4 font-display text-[30px] leading-tight text-[#557f79]">{result.ancient.fused.primary?`主方向 ${result.ancient.fused.primary.labelZh} ${result.ancient.fused.primary.centerDeg}°`:firstDirection?`单证方向 ${firstDirection.labelZh} ${firstDirection.centerDeg}°`:"证据未成向"}</p><PublicationCopy>{result.ancient.fused.rationaleZh}</PublicationCopy><div className="mt-4 grid grid-cols-2 gap-3 text-[12px] leading-5 text-[#565162]"><p>圆周合度<br/><strong>R = {result.ancient.fused.resultantLength.toFixed(3)}</strong></p><p>候选扇区<br/><strong>{result.ancient.fused.primary?sectorLabel(result.ancient.fused.primary.sector):firstDirection?sectorLabel(firstDirection.sector):"尚未成域"}</strong></p></div><p className="mt-4 text-[11px] leading-5 text-[#696473]">单证可以交付其原始方位与证据链，但只有两套以上独立原典模块通过合度门，才形成唯一主向。</p></PublicationPage>
