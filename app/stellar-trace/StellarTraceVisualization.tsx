  ...
  return (
    <section className={live ? "overflow-hidden border border-white/15 bg-[linear-gradient(145deg,rgba(34,50,91,.88),rgba(31,28,70,.76))] p-5 shadow-[0_24px_80px_rgba(0,0,0,.28)] backdrop-blur-xl sm:p-8" : "mt-5"} data-stellar-visualization={mode} data-phase={phase} aria-label={`星迹四阶段演算，当前为${stages[phase - 1]}`}>
      ...
      <div className={`grid items-center gap-5 ${live ? "mt-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(240px,.75fr)]" : "sm:grid-cols-[310px_1fr]"}`}>
        ...
        <div className={live ? "text-bone" : "text-[#454151]"}><p className={`text-[11px] tracking-[.24em] ${live ? "text-amber" : "text-[#557f79]"}`}>{String(phase).padStart(2, "0")} · {stages[phase - 1]}</p><p className="mt-3 font-display text-2xl">{phase < 2 ? "九域初定" : phase < 3 ? "原典证据入位" : phase < 4 ? `原典合度 · ${status}` : result.priority.available ? `原典候选方位 · ${result.priority.primaryDirectionZh}` : "原典证据尚未成向"}</p><p className={`mt-3 text-xs leading-6 ${live ? "text-bone-dim" : "text-[#696473]"}`}>{phase < 2 ? "九个天体按当前真实黄经落位；这一层只记录天文事实。" : phase < 3 ? "奇门、六壬、太乙、六爻仅在真实规则与输入成立时入位。" : phase < 4 ? "只有带来源、可复算的真实起局结果才能进入原典圆周合度。" : result.priority.basisZh}</p>{phase >= 4 && <div className={`mt-5 grid gap-2 text-xs ${live ? "text-bone-dim" : "text-[#565162]"}`}><p>原典覆盖 · <strong>{result.ancient.fused.usedSystems.length} / 4</strong></p><p>候选方位 · <strong>{result.priority.available ? `${result.priority.primaryDirectionZh} ${result.priority.primaryBearing}°` : "未成"}</strong></p><p>现实方向核验 · <strong>{result.ancient.realityValidation.noteZh}</strong></p></div>}</div>
      </div>
    </section>
  );
}
