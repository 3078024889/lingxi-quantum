    ...
    <section className={live ? "overflow-hidden border border-white/15 bg-[linear-gradient(145deg,rgba(34,50,91,.88),rgba(31,28,70,.76))] p-5 shadow-[0_24px_80px_rgba(0,0,0,.28)] backdrop-blur-xl sm:p-8" : "mt-5"} data-stellar-visualization={mode} data-phase={phase} aria-label={`星迹四阶段演算，当前为${stages[phase - 1]}`}>
      ...
      <div className={`grid items-center gap-5 ${live ? "mt-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(240px,.75fr)]" : "sm:grid-cols-[310px_1fr]"}`}>
        <svg viewBox="0 0 360 360" className="mx-auto w-full max-w-[430px]" role="img" aria-label="九域与四层证据圆周汇流图">
          ...
          {phase >= 4 && <g opacity=".72"><path d={`M 180 180 L ${polar(centerDirection - sectorSpread / 2, 155).x} ${polar(centerDirection - sectorSpread / 2, 155).y} A 155 155 0 0 1 ${polar(centerDirection + sectorSpread / 2, 155).x} ${polar(centerDirection + sectorSpread / 2, 155).y} Z`} fill="rgba(145,121,182,.22)" stroke="rgba(222,201,151,.62)"/><text x="180" y="334" textAnchor="middle" fontSize="10" fill={live ? "#cfc8bd" : "#696473"}>主核验扇区 · {result.priority.primaryDirectionZh} {result.priority.primaryBearing}°</text></g>}
        </svg>
        <div className={live ? "text-bone" : "text-[#454151]"}><p className={`text-[11px] tracking-[.24em] ${live ? "text-amber" : "text-[#557f79]"}`}>{String(phase).padStart(2, "0")} · {stages[phase - 1]}</p><p className="mt-3 font-display text-2xl">{phase < 2 ? "九域初定" : phase < 3 ? "诸证入位" : phase < 4 ? `合度 · ${status}` : `主核验方位 · ${result.priority.primaryDirectionZh}`}</p><p className={`mt-3 text-xs leading-6 ${live ? "text-bone-dim" : "text-[#696473]"}`}>{phase < 2 ? "九个天体按当前真实黄经落位。" : phase < 3 ? "四层投影与现实方向锚点分别进入圆周。" : phase < 4 ? "集中度用于区分全局收敛、局部成簇与探索排序。" : result.priority.basisZh}</p>{phase >= 4 && <div className={`mt-5 grid gap-2 text-xs ${live ? "text-bone-dim" : "text-[#565162]"}`}><p>主核验方位 · <strong>{result.priority.primaryDirectionZh} {result.priority.primaryBearing}°</strong></p><p>推演状态 · <strong>{status}</strong></p><p>距离与坐标 · <strong>{result.distance.rangeKm ? `${result.distance.rangeKm[0]}—${result.distance.rangeKm[1]} km${result.candidateCenter ? ` · ${result.candidateCenter.lat}, ${result.candidateCenter.lon}` : ""}` : "现实距离证据不足，止于方位层"}</strong></p></div>}</div>
      </div>
    </section>
  );
}
