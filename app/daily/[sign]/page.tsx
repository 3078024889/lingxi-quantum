    ...
    <>
      ...
      <main className="pt-24">
        <div className="mx-auto max-w-xl px-6 py-16">
          <div className="flex items-center justify-between gap-3 lx11-legacy-panel-daily px-6 py-4 text-center">
            ...
            <DownloadResultPdfButton
              ...
              bgColorRgb={[14, 16, 42]}
              bgColorHex="#0e102a"
              colorClass="shrink-0 border-[var(--lx-line-strong)] text-[var(--lx-ink)] hover:border-[var(--lx-line-strong)] hover:bg-[var(--lx-soft)]"
            />
          </div>

          <div id="daily-result" className="mt-4">

          <div className="mt-6 flex flex-col items-center lx11-legacy-panel-daily p-8 text-center">
            <span className="font-display text-5xl text-[var(--lx-ink)]">{sign.glyph}</span>
            <h1 className="mt-3 font-display text-3xl font-light text-[var(--lx-ink)]">
              <Bi zh={`${sign.zh}座 · 今日潮汐`} en={`${sign.en} · Today’s Tide`} />
            </h1>
            <p className="mt-2 text-xs text-[var(--lx-faint)]">{todayLabel}</p>
            ...
            <div className="mt-5 overflow-hidden rounded-sm border border-[var(--lx-line)]" style={{ maxWidth: 220 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/images/daily/daily.jpg" alt="Today’s Tide" className="block w-full" />
            </div>
          </div>

          <div className="mt-4 lx11-legacy-panel-daily p-6">
            <p className="text-xs uppercase tracking-widest2 text-[var(--lx-ink)]"><Bi zh="今日场域解读" en="Today's Field Reading" /></p>
            <p className="mt-2 text-base leading-8 text-[var(--lx-muted)]">
              <Bi zh={fortuneZh || fallbackZh} en={fortuneEn || fallbackEn} />
            </p>

            <div className="mt-5 border-t border-[var(--lx-line)] pt-5">
              <div className="flex items-center justify-between">
                <p className="text-xs uppercase tracking-widest2 text-[var(--lx-ink)]"><Bi zh="能量潮汐" en="Energy Tide" /></p>
                <p className="text-xs text-[var(--lx-muted)]">{tide}/100</p>
              </div>
              <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-white/10">
                <div className="h-full rounded-full bg-gradient-to-r from-lattice to-amber" style={{ width: `${tide}%` }} />
              </div>
              <p className="mt-2 text-xs leading-6 text-[var(--lx-faint)]">
                <Bi
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-[11px] text-[var(--lx-faint)]">
              <span className="rounded-sm border border-[var(--lx-line)] px-2 py-1">
                <Bi zh={`当日守护星：${ruler.zh}`} en={`Day Ruler: ${ruler.en}`} />
              </span>
              {retro.length > 0 && (
                <span className="rounded-sm border border-amber/25 px-2 py-1 text-[var(--lx-ink)]">
          ...
          <div className="mt-6 grid grid-cols-6 gap-2 sm:grid-cols-12">
            {ZODIAC_SIGNS.map((s) => (
              <Link
                key={s.slug}
                href={`/daily/${s.slug}`}
                className={`flex flex-col items-center gap-1 rounded-sm border py-3 text-lg transition ${s.slug === sign.slug ? "border-[var(--lx-line-strong)] bg-[var(--lx-soft)] text-[var(--lx-ink)]" : "border-[var(--lx-line)] bg-[var(--lx-panel)] text-[var(--lx-muted)] hover:border-[var(--lx-line-strong)]"}`}
              >
                {s.glyph}
              </Link>
            ))}
          ...
          <div className="mt-10 border-t border-[var(--lx-line)] pt-6 text-center">
            ...
            <a
              href="/life-map"
              className="mt-4 inline-block border border-[var(--lx-line-strong)]/45 px-5 py-2 font-display text-xs uppercase tracking-widest2 text-[var(--lx-ink)] transition hover:border-[var(--lx-line-strong)] hover:text-[var(--lx-ink)]"
            >
              <Bi zh="继续探索：完整生命图谱 →" en="Continue exploring: Full Life Map →" />
            </a>
          </div>

          <p className="mt-6 text-center text-xs text-[var(--lx-faint)]">
