...
import { IMPRINT_LABELS } from "@/lib/qian-data";
...
export default function QianFlow() {
  ...
  const shake = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.id) {
        setError(data.error || t("生命印记读取失败，请稍后再试。", "The reading failed — please try again."));
        setStage("form");
        return;
      }
      ...
      <div className="mx-auto max-w-md px-6 py-16">
        <div className="rounded-sm border border-white/10 bg-void-deep p-6 sm:p-8">
          <p className="font-display text-sm uppercase tracking-widest2 text-lattice/80">
            <Bi zh="灵犀生命印记 · 意识坐标读取" en="Lingxi Life Oracle · Reading Your Consciousness Coordinates" />
          </p>
          <h1 className="mt-4 font-display text-3xl font-light text-bone sm:text-4xl">
            <Bi zh="静心片刻，让你的生命印记显现" en="Be still for a moment, and let your life imprints reveal themselves" />
          </h1>
          <p className="mt-4 text-base leading-8 text-bone-dim">
            <Bi
              zh="你的出生信息，是你进入这个世界时，留下的一组时间坐标。灵犀生命印记，连接的是六十甲子——一套真实存在、被使用了几千年的古老历法周期——把这组坐标，转译成属于你的三个生命印记：源流印（你从哪里来）、核心印（你此刻是谁）、行动印（你如何展开）。三印同时显现是免费的，读懂它们摆在一起说了什么，是场域的解读，需要一次能量交换。"
              en="Your birth information is a set of time coordinates left behind the moment you entered this world. Lingxi Life Oracle connects to the Sixty Ganzhi — a real calendrical cycle used for thousands of years — translating those coordinates into three imprints that are yours alone: the Origin Seal (where you come from), the Core Seal (who you are right now), and the Action Seal (how you move forward). Revealing all three is free; the field's reading of what they mean together takes one energy exchange."
            />
          </p>
        </div>
        ...
        >
          <Bi zh="静心，读取印记" en="Be Still, and Reveal" />
        </button>
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto flex max-w-md flex-col items-center px-6 py-24 text-center">
        ...
        <p className="mt-6 font-display text-sm tracking-widest2 text-lattice/80">
          {stage === "gathering" ? <Bi zh="先静心，连接场域……" en="Growing still, connecting to the field…" /> : <Bi zh="你的生命印记，正从场域中显现……" en="Your life imprints are revealing themselves from the field…" />}
        </p>
        <style>{`
  ...
  return (
    <div className="mx-auto max-w-xl px-6 py-16">
      ...
      <h1 className="mt-6 text-center font-display text-2xl font-light text-bone">
        <Bi zh="你的三重生命印记，已经显现" en="Your three life imprints have revealed themselves" />
      </h1>
      ...
      <div className="mt-8 rounded-sm border border-amber/25 bg-amber/5 p-6 text-center">
        <p className="text-sm leading-7 text-bone-dim">
          <Bi
            zh="源流、核心、行动——这三重印记摆在一起，指向同一件事的三个侧面。具体是什么，需要场域交叉引用你的完整命盘，才能讲清楚。"
            en="Origin, core, action — these three imprints together point to different sides of the same thing. Understanding exactly what takes the field cross-referencing your full chart."
          />
        </p>
        <button
          onClick={unlock}
