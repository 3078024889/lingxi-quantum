...
// 跟 ReportRow.tsx（生命图谱那份）是同一套交互，只是换成关系共振图谱
// 对应的接口和链接——之前这里只有一个纯 <Link>，用户没有办法删除。
export default function RelationshipReportRow({
  id,
  title,
  date,
}: {
  ...
  return (
    <div className="flex items-center gap-2 rounded-sm border border-white/10 bg-void-deep px-2 py-1">
      <Link href={`/relationship/full?id=${id}`} className="flex flex-1 items-center justify-between px-3 py-2 transition hover:opacity-80">
        <span className="font-display text-lattice">{title}</span>
