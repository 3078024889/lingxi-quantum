...
export default function WealthReportView({ id }: { id: string }) {
  ...
  if (status === "checking") {
    return (
      <div className="mx-auto max-w-md px-6 py-24 text-center">
        <div className="lx-glass-wealth px-6 py-10">
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16">
      <div className="flex items-center justify-between lx-glass-wealth px-6 py-4">
      ...
      <div ref={reportRef} className="mt-4">
        ...
        {sections.map((content, i) => {
          const bg = `/images/wealth-full/page-${(i % 10) + 1}.jpg`;
          const title = SECTION_TITLES[i] ?? { titleZh: `第${i + 1}段`, titleEn: `Section ${i + 1}` };
          return (
            <div
              key={i}
              className="relative mt-4 overflow-hidden rounded-sm"
              style={{ backgroundColor: "#14100a", backgroundImage: `linear-gradient(rgba(9,37,31,0.55), rgba(9,37,31,0.55)), url(${bg})`, backgroundSize: "cover", backgroundPosition: "center" }}
            >
              <div className="p-8">
