...
type Tributary = { productId: string; nameZh: string; nameEn: string; noteZh?: string; completed: boolean; assessmentCompleted?: boolean; evidenceReady?: boolean; needsRetest?: boolean; accessActive?: boolean; completedAt?: string | null; reportId?: string | null };
type Subject = { subject: { subjectId: string; displayName: string; birthDate?: string }; completed: number };
type Progress = { authenticated: boolean; ready: boolean; completed: number; subject?: Subject["subject"]; subjects?: Subject[]; tributaries: Tributary[]; submissionId?: string; archivedSubmissionId?: string; windowEndsAt?: string | null; blockedReason?: "identity-mismatch"|"legacy-evidence-missing"|"outside-365-days"|"coverage-incomplete"; subjectSelectionRequired?: boolean; error?: string };
...
export default function ArchetypeProgress() {
  ...
  return <section className="lx-glass mx-auto max-w-4xl border border-lattice/25 p-7 sm:p-10">
    ...
    {(data.subjects?.length ?? 0) > 0 && <label className="mt-7 block text-sm text-bone-dim">核对报告主体<select value={selected} onChange={(event) => { setSelected(event.target.value); if(event.target.value) void load(event.target.value); }} className="mt-2 w-full border border-white/15 bg-[#07102c] px-4 py-3 text-bone"><option value="">请选择同一姓名主体</option>{data.subjects!.map((item) => <option key={item.subject.subjectId} value={item.subject.subjectId}>{item.subject.displayName} · {item.completed}/8</option>)}</select></label>}
    ...
    <div className="mt-7 grid gap-3 sm:grid-cols-2">{(data.tributaries ?? []).map((item) => <article key={item.productId} className={`border p-4 ${item.needsRetest ? "border-amber/45 bg-amber/[.06]" : item.completed ? "border-lattice/45 bg-lattice/[.07]" : "border-white/10 bg-white/[.025]"}`}><div className="flex items-start justify-between gap-4"><div><h3 className="font-display text-lg text-bone">{item.nameZh}</h3><p className="mt-1 text-[11px] tracking-[.12em] text-bone-mute">{item.nameEn}</p></div><span className={`text-xs ${item.needsRetest ? "text-amber" : item.completed ? "text-lattice" : "text-bone-mute"}`}>{item.needsRetest ? "旧版缺证据 · 请同名重测" : item.completed ? "已完成" : item.assessmentCompleted ? "待恢复权限" : "未开启"}</span></div>{item.completedAt && <p className="mt-3 text-xs text-bone-mute">采用版本 · {new Date(item.completedAt).toLocaleString("zh-CN")}</p>}{item.noteZh && <p className="mt-2 text-xs leading-5 text-bone-dim">{item.noteZh}</p>}</article>)}</div>
    {data.ready && reportId ? <a href={`/mini-report?id=${encodeURIComponent(reportId)}`} className="mt-7 inline-flex border border-lattice/60 bg-lattice/10 px-6 py-4 text-sm tracking-[.14em] text-lattice">展开完整生命原型报告 →</a> : <div className="mt-7 border-l border-lattice/50 bg-white/[.025] px-5 py-4 text-sm leading-7 text-bone-dim">{data.subjectSelectionRequired?"此账户已有多个完成八流的姓名主体。请选择姓名核对，系统不会自行合并或猜测。":data.blockedReason==="legacy-evidence-missing"?"八流数量已齐，但至少一份旧报告没有保存底层证据叶，系统已停止补写。请用同一姓名重新完成该支流后再成卷。":data.blockedReason==="identity-mismatch"?"八份报告的姓名主体未通过完全一致核验，暂不生成生命原型。":data.blockedReason==="outside-365-days"?"八份报告未落在同一 365 天有效窗口内，暂不生成生命原型。":"自第一条支流开启之日起，365天内完成八项同主体场域精测。八流全部解锁后，系统才会读取底层证据并生成完整生命原型档案。"}</div>}
