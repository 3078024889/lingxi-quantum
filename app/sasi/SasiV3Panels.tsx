...
export function SasiAccountCenter({ lang, dark, accountEmail, projectCount, onOpenWorks, onOpenModels }: { lang: Lang; dark: boolean; accountEmail: string | null; projectCount: number; onOpenWorks: ()=>void; onOpenModels: ()=>void }) {
  ...
  return <section>
    ...
    <div className={`sasi-account-plans border ${panel}`}><div><p className="sasi-v3-kicker">MEMBERSHIP ARCHITECTURE</p><h2>{tr(lang,"会员购买工作能力，制作额度承担真实生成","Membership unlocks workflow value; production credits fund real generation")}</h2><p>{tr(lang,"套餐权益与外部模型消耗分开，避免用户为同一项能力重复付费。以下仅是产品结构预览，支付、续费与退款链路完成前不会开放购买。","Plan benefits stay separate from provider consumption, avoiding double charging. This is a product preview only; purchases remain closed until payment, renewal and refund flows are verified.")}</p></div><div className="sasi-account-plan-grid"><article><small>FREE</small><h3>{tr(lang,"开始创作","Start creating")}</h3><p>{tr(lang,"项目整理、成熟模板、自带模型连接与基础导出。","Project organization, mature templates, BYOK connections and basic exports.")}</p></article><article className="featured"><small>PROFESSIONAL</small><h3>{tr(lang,"把方案推进到交付","Move plans to delivery")}</h3><p>{tr(lang,"专业导演流程、连续性检查、多模型编排、版本记录与结构化审核。","Professional directing workflows, continuity checks, orchestration, version history and structured review.")}</p></article><article><small>STUDIO</small><h3>{tr(lang,"为团队建立生产系统","A production system for teams")}</h3><p>{tr(lang,"共享资产、成员权限、审批节点、团队工作区与优先支持；协作后端完成后开放。","Shared assets, member roles, approvals, team workspaces and priority support—after collaboration services are live.")}</p></article></div><footer><span>{tr(lang,"方案预览 · 暂不可购买","Plan preview · not yet purchasable")}</span><span>{tr(lang,"余额与用量继续在独立板块管理","Balance and usage remain in their dedicated section")}</span></footer></div>
  </section>;
}
