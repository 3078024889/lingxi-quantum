...
export function SasiAccountCenter({ lang, dark, accountEmail, projectCount, onOpenWorks, onOpenModels }: { lang: Lang; dark: boolean; accountEmail: string | null; projectCount: number; onOpenWorks: ()=>void; onOpenModels: ()=>void }) {
  ...
  return <section>
    ...
    <div className={`sasi-account-plans border ${panel}`}><div><p className="sasi-v3-kicker">FROM IDEA TO CREATION</p><h2>{tr(lang,"说出你的目标，让作品开始生长","Give your idea a direction")}</h2><p>{tr(lang,"带来剧本、灵感或产品目标。SASI 为你展开制作方案与预算，你确认后，沿着同一个项目继续创作。","Bring a script, idea or product goal. Review a production plan and budget, then continue creating in one project.")}</p></div><div className="sasi-account-plan-grid"><article><small>01</small><h3>{tr(lang,"告诉 SASI 你想创造什么","Tell SASI what you want to create")}</h3><p>{tr(lang,"剧本、集数、每集时长，或一个需要解决的真实问题。","Bring your script, episode count and duration, or a real problem to solve.")}</p></article><article className="featured"><small>02</small><h3>{tr(lang,"看看方案与预算","Review the plan and budget")}</h3><p>{tr(lang,"看清这次会得到什么；想调整时，修改目标或制作规格。","See what you will receive. Adjust your goal or production specification when needed.")}</p></article><article><small>03</small><h3>{tr(lang,"确认，让创作继续","Approve and keep creating")}</h3><p>{tr(lang,"确认可执行的任务后开始制作，成果与版本留在你的作品库。","Start an available task after approval. Keep its results and versions in your library.")}</p></article></div><footer><span>{tr(lang,"先看方案与预算，再决定开始","Review the plan and budget before starting")}</span><span>{tr(lang,"每次修改，都承接上一次创作","Each revision continues the work before it")}</span></footer></div>
  </section>;
}
