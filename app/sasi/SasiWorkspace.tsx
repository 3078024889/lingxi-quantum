  ...
  async function prepare(kind: "code" | "drama") {
    ...
    try {
      ...
      if (kind === "drama") {
        ...
        setNotice(copy(lang,
          `项目 ${result.project.id.slice(0, 8)} 已建立：${plan.episodes} 集 × 约 ${plan.secondsPerEpisode} 秒。${uploadedAssets ? `已接收 ${uploadedAssets} 项资产${reviewAssets ? `，其中 ${reviewAssets} 项等待深度安全审阅` : "并完成安全索引"}。` : ""}${result.generationBlocked ? `当前预算尚差 ${formatRmb(result.quote.gap)}。` : "制作图谱已保存；获得最终授权后才会预留余额。"}`,
          `Project ${result.project.id.slice(0, 8)} is live: ${plan.episodes} episode(s) × about ${plan.secondsPerEpisode}s. ${uploadedAssets ? `${uploadedAssets} asset(s) received${reviewAssets ? `; ${reviewAssets} await deep security review` : " and safely indexed"}. ` : ""}${result.generationBlocked ? `The current budget is short by ${formatRmb(result.quote.gap)}.` : "The production graph is saved; balance is reserved only after final authorization."}`,
        ));
      } else {
          ...
          {view === "drama" && (
            <section>
              ...
              <div className="mt-6 grid gap-4 xl:grid-cols-[1fr_360px]">
                <div className={`rounded-3xl border p-6 ${panel}`}><textarea value={script} onChange={(event) => setScript(event.target.value)} onPaste={handlePaste} placeholder={copy(lang, "写下创意，或导入剧本、小说、人物图、故事板、音频和已有视频……", "Write an idea or import a script, novel, character image, storyboard, audio or existing video…")} className="min-h-36 w-full resize-none bg-transparent text-base leading-7 outline-none"/><UploadHub lang={lang} files={files} onAdd={addFiles} onRemove={(id) => setFiles((current) => current.filter((file) => file.id !== id))} onNotice={setNotice} /><div className="mt-5 grid gap-3 sm:grid-cols-2"><label className="text-xs opacity-60">{copy(lang, "目标总时长（5–600秒）", "Total duration (5–600 sec)")}<input type="number" min={5} max={600} value={seconds} onChange={(event) => setSeconds(Number(event.target.value))} className="mt-2 w-full rounded-xl border border-current/15 bg-transparent px-3 py-3 text-base outline-none"/></label><label className="text-xs opacity-60">{copy(lang, "集数（留空由 SASI 建议）", "Episodes (optional)")}<input type="number" min={1} max={200} value={episodes} onChange={(event) => setEpisodes(event.target.value)} placeholder={copy(lang, "动态分析，不预设", "Dynamic, not preset")} className="mt-2 w-full rounded-xl border border-current/15 bg-transparent px-3 py-3 text-base outline-none"/></label><label className="text-xs opacity-60">{copy(lang, "项目预算上限（人民币）", "Project budget cap (RMB)")}<input type="number" min={0} step="1" value={budget} onChange={(event) => setBudget(Number(event.target.value))} className="mt-2 w-full rounded-xl border border-current/15 bg-transparent px-3 py-3 text-base outline-none"/></label><label className="text-xs opacity-60">{copy(lang, "制作规格", "Production grade")}<select value={quality} onChange={(event) => setQuality(event.target.value as SasiQuality)} className={`mt-2 w-full rounded-xl border border-current/15 px-3 py-3 text-base outline-none ${dark ? "bg-[#11151b]" : "bg-white"}`}>{SASI_QUALITY_TIERS.map((item) => <option key={item.id} value={item.id}>{copy(lang, item.zh, item.en)}</option>)}</select></label></div><p className="mt-4 text-xs leading-5 opacity-50">{copy(lang, "SASI Auto 将按叙事价值调度制作能力；无需选择模型或管理技术账户。", "SASI Auto allocates production capability by narrative value; no model or technical account selection is required.")}</p><button disabled={preparing} onClick={() => prepare("drama")} className="mt-6 w-full rounded-xl bg-[#e04d70] py-3 text-sm font-semibold text-white disabled:cursor-wait disabled:opacity-50">{preparing ? copy(lang, "正在建立项目…", "Creating project…") : copy(lang, "建立项目并形成提案", "Create project & proposal")}</button></div>
                <div className={`rounded-3xl p-6 ${dark ? "bg-[#d9ff73] text-black" : "bg-[#151515] text-white"}`}><p className="text-xs uppercase tracking-[.2em] opacity-55">PRODUCTION ENVELOPE</p><p className="mt-5 text-3xl font-semibold">{formatRmb(quote.amountFen)} <span className="text-sm font-normal opacity-50">{copy(lang, "人民币预算", "RMB budget")}</span></p><p className="mt-1 text-sm opacity-60">{seconds}s · {copy(lang, SASI_QUALITY_TIERS.find((tier) => tier.id === quality)?.zh ?? "灵感验证", SASI_QUALITY_TIERS.find((tier) => tier.id === quality)?.en ?? "Concept")}</p><div className="my-5 border-t border-current/15"/><p className="text-sm">{copy(lang, "SASI Auto · 智能统筹", "SASI Auto · intelligent orchestration")}</p><dl className="mt-5 space-y-2 text-xs"><div className="flex justify-between"><dt className="opacity-55">{copy(lang, "建议人民币预算", "Proposed RMB budget")}</dt><dd>{formatRmb(quote.amountFen)}</dd></div><div className="flex justify-between"><dt className="opacity-55">{copy(lang, "投入边界", "Allocation boundary")}</dt><dd>{formatRmb(quote.budget)}</dd></div><div className="flex justify-between"><dt className="opacity-55">{copy(lang, "尚需配置", "Remaining allocation")}</dt><dd>{formatRmb(quote.gap)}</dd></div></dl><div className={`mt-5 rounded-xl p-3 text-xs leading-5 ${quote.canConfirm ? "bg-emerald-500/15" : "bg-amber-500/20"}`}>{quote.canConfirm ? copy(lang, `当前投入边界支持本次制作。最终授权后，最多预留 ${formatRmb(quote.amountFen)}。`, `The current allocation supports this production. Final authorization reserves no more than ${formatRmb(quote.amountFen)}.`) : copy(lang, "当前投入边界与制作规格尚未对齐。可调整时长、镜头策略或制作规格，系统会重新推演。", "The allocation and production grade are not yet aligned. Adjust duration, shot strategy or grade for a new study.")}</div><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><button className="rounded-lg border border-current/15 py-2" onClick={() => setBudget(quote.amountFen / 100)}>{copy(lang, "对齐投入边界", "Align allocation")}</button><button className="rounded-lg border border-current/15 py-2" onClick={() => setSeconds(Math.max(5, Math.floor(seconds * .7)))}>{copy(lang, "重构篇幅", "Refine duration")}</button></div></div>
              </div>
