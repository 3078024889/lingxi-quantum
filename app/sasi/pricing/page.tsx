...
export default function SasiPricingPage() {
  ...
  return <><Nav /><main className="mx-auto max-w-6xl px-6 py-20">
    ...
    <div className="mt-7 flex flex-wrap gap-4"><Link className="rounded-xl border px-5 py-3" href="/sasi/drama?view=director">带来我的剧本 →</Link><Link className="rounded-xl border px-5 py-3" href="/sasi/drama?view=billing#topup">进入余额与充值 →</Link></div>
    ...
    <section className="mt-12" id="recharge">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">{packs.map(pack => <Link href="/sasi/drama?view=billing#topup" key={pack.id} className="rounded-xl border p-5"><b className="text-2xl">¥{pack.priceRmb.toLocaleString("zh-CN")}</b><p className="mt-2 text-sm">人民币创作余额</p></Link>)}</div>
      <Link href="/sasi/drama?view=billing#topup" className="mt-6 inline-block rounded-xl border px-6 py-3">查看账户与充值状态 →</Link>
    </section>
