...
export default function CangXuanDirectorStudio({ lang, dark, accountEmail, onEnterProduction }: Props) {
  ...
  return <section>
    ...
    <div id="cangxuan-brief" className="mt-5 grid scroll-mt-24 gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
      ...
      <aside className={`rounded-3xl border p-6 ${panel}`}><p className="text-xs uppercase tracking-[.18em] text-[#7657ff]">导演步骤</p><ol className="mt-5 space-y-4 text-sm">{[t("项目理解与叙事约束","Brief & constraints"),t("人物圣经与视觉锁","Character bible"),t("分集结构与情绪曲线","Episode architecture"),t("分镜与摄影语言","Shots & cinematography"),t("连续性审校","Continuity audit"),t("进入可选视频生产","Optional production")].map((v,i)=><li key={v} className="flex gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#7657ff]/12 text-xs text-[#7657ff]">{i+1}</span><span className="pt-1 opacity-70">{v}</span></li>)}</ol></aside>
    </div>
