...
import { buildDirectorBlueprint, type DirectorBlueprint, type DirectorBrief } from "@/lib/sasi/cangxuan-director";

type Props = { lang: "zh" | "en"; dark: boolean; onEnterProduction: (story: string) => void };

const initialBrief: DirectorBrief = { title: "", premise: "", protagonist: "", medium: "motion-comic", genre: "古装复仇", episodes: 12, secondsPerEpisode: 60 };
...
export default function CangXuanDirectorStudio({ lang, dark, onEnterProduction }: Props) {
  ...
  return <section>
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]">
      <div className={`rounded-3xl border p-6 ${panel}`}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-xs opacity-65">{t("创作形态", "Medium")}<select value={brief.medium} onChange={e=>setBrief({...brief,medium:e.target.value as DirectorBrief["medium"]})} className={`mt-2 w-full rounded-xl border border-current/15 px-4 py-3 text-sm outline-none ${dark?"bg-[#11151b]":"bg-white"}`}><option value="motion-comic">AI 漫剧</option><option value="live-action">AI 真人短剧</option></select></label>
          <label className="text-xs opacity-65">{t("类型", "Genre")}<select value={brief.genre} onChange={e=>setBrief({...brief,genre:e.target.value})} className={`mt-2 w-full rounded-xl border border-current/15 px-4 py-3 text-sm outline-none ${dark?"bg-[#11151b]":"bg-white"}`}>{["古装复仇","都市情感","仙侠成长","悬疑反转","品牌广告"].map(v=><option key={v}>{v}</option>)}</select></label>
    ...
    {blueprint && <div className="mt-7 space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2"><BlueprintList title={t("世界规则","World rules")} items={blueprint.worldRules} panel={panel}/><BlueprintList title={t("连续性锁","Continuity locks")} items={blueprint.continuityLocks} panel={panel}/></div>
