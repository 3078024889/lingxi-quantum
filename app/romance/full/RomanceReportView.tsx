  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      await exportArchivePdf({
        ...
        bodyImages: Array.from({ length: 11 }, (_, k) => `/images/romance-full/page-${k + 1}.png`),
        endImage: "/images/romance-full/page-11.png",
      });
    } catch (e) {
    ...
    <div className="mx-auto max-w-4xl px-3 py-16 sm:px-6">
      ...
      <div ref={reportRef} className="lx-report-tone-light lx-theme-romance mt-4">
        ...
        {sections.map((content, i) => {
          const bg = `/images/romance-full/page-${(i % 11) + 1}.png`;
          const title = SECTION_TITLES[i] ?? { titleZh: `第${i + 1}段`, titleEn: `Section ${i + 1}` };
          return (
            <section
              key={i}
              className="lx-publication-page lx-page-layout-center relative mt-6 flex items-center overflow-hidden rounded-sm"
            ...
            >
              <div className="lx-report-glass lx-report-glass-readable my-10 px-8 py-10 sm:px-10 sm:py-12">
                <p className="font-display text-[11px] uppercase tracking-[0.34em] text-[#8C7FA8]">
                  ROMANCE FIELD · {String(i + 1).padStart(2, "0")} / {String(sections.length).padStart(2, "0")}
                </p>
