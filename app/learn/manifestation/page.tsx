    ...
    <>
      ...
      <main className="px-6 pb-24 pt-28">
        <article className="mx-auto max-w-2xl">
          <div className="bg-[var(--lx-panel)] px-8 py-10">
          <p className="font-display text-sm uppercase tracking-widest2 text-[var(--lx-ink)]">显化 · Manifestation</p>
          ...
          {/* 定义块（便于被引用） */}
          <div className="mt-8 rounded-sm border border-[var(--lx-line)] bg-[var(--lx-soft)] p-6">
            <p className="text-lg leading-9 text-[var(--lx-ink)]">
              <Bi
          ...
          <div className="mt-10 space-y-6 text-lg leading-9 text-[var(--lx-muted)]">
            <h2 className="font-display text-2xl font-light text-[var(--lx-ink)]"><Bi zh="显化的原理" en="How manifestation works" /></h2>
            <p>
              <Bi
            ...
            <h2 className="font-display text-2xl font-light text-[var(--lx-ink)]"><Bi zh="每日显化方法（4 步）" en="A daily manifestation method (4 steps)" /></h2>
            <p>
              <Bi
            ...
            <h2 className="font-display text-2xl font-light text-[var(--lx-ink)]"><Bi zh="常见误区" en="Common pitfalls" /></h2>
            <p>
              <Bi
          ...
          <div className="mt-14">
            <h2 className="font-display text-2xl font-light text-[var(--lx-ink)]"><Bi zh="常见问题" en="FAQ" /></h2>
            <div className="mt-6 space-y-5">
              {faq.map((f, i) => (
                <div key={i} className="rounded-sm border border-[var(--lx-line)] bg-[var(--lx-panel)] p-6">
                  <p className="font-display text-lg text-[var(--lx-ink)]">{f.q}</p>
                  <p className="mt-3 leading-8 text-[var(--lx-muted)]">{f.a}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* CTA */}
          <div className="mt-14 rounded-sm border border-[var(--lx-line)] bg-[var(--lx-soft)] p-7 text-center">
            <p className="font-display text-xl text-[var(--lx-ink)]"><Bi zh="把显化变成每日练习" en="Make manifestation a daily practice" /></p>
            <p className="mt-3 leading-8 text-[var(--lx-muted)]"><Bi zh="灵犀场的「显化签到」陪你每天进入「已经拥有」的状态，并给你来自场的回响。" en="Lingxi's daily check-in helps you enter the 'already have it' state each day, with a reflection from the field." /></p>
            <Link href="/live-as" className="mt-6 inline-block bg-lattice px-10 py-4 font-display text-sm uppercase tracking-widest2 text-[var(--lx-bg)] transition hover:bg-amber">
              <Bi zh="进入显化签到" en="Open the check-in" />
            </Link>
          </div>

          <p className="bg-[var(--lx-panel)] mt-10 rounded-sm px-6 py-4 text-sm text-[var(--lx-muted)]">
            <Bi zh="延伸：" en="Related: " />
            <Link href="/practice" className="text-[var(--lx-ink)] hover:text-[var(--lx-ink)]"><Bi zh="从觉察到实践" en="From awareness to practice" /></Link>
            {" · "}
            <Link href="/learn/dream" className="text-[var(--lx-ink)] hover:text-[var(--lx-ink)]"><Bi zh="如何解梦" en="How to interpret dreams" /></Link>
          </p>
        </article>
      </main>
      <Footer />
