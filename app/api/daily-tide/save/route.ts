...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (insertErr || !submission) {
      console.error("[daily-tide save] 写入失败，Supabase 原始错误:", insertErr);
      if (insertErr?.code === "42P01") {
        return NextResponse.json(
          { error: "保存失败：数据库里还没有这张表。需要在 Supabase 后台的 SQL Editor 里跑一次 SQL-v237-daily-tide-table.sql。" },
          { status: 500 }
        );
      }
    ...
    console.error("[daily-tide save] 计算失败:", e);
