...
export async function GET(req:Request){
  ...
  const {data:rows,error}=await admin.from("balance_withdrawals")
    .select("id,user_id,order_id,provider,currency,amount_minor,status")
  ...
  for(const w of rows||[]){
    ...
    const orderAmountMinor=w.currency==="USD"
    ...
    try{
      const attempt=await executeProviderRefund({
        ...
        orderAmountMinor,
        refundAmountMinor:Number(w.amount_minor),
      });
      if(attempt.state==="completed"){
        ...
        const released=await admin.rpc("release_balance_withdrawal",{
          p_withdrawal_id:w.id,
