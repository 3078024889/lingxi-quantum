...
export async function GET(req:Request){
 ...
 let deletedObjects=0,deletedNotes=0;
 for(const n of notes||[]){
   const {data:files}=await admin.from("burn_files").select("object_key").eq("note_id",n.id);
   let ok=true;
   for(const f of files||[]){if(await r2Delete(f.object_key))deletedObjects++;else ok=false}
   if(ok){const d=await admin.from("burn_notes").delete().eq("id",n.id);if(!d.error)deletedNotes++}
 }
 try { await admin.rpc("cleanup_ephemeral_privacy_data"); } catch { /* cleanup is best-effort */ }
