...
// v228：桃花磁场指数从"免费单页快测"升级出一条付费深度报告的产品线，
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (insertErr || !submission) {
      console.error("[romance save] 写入失败，Supabase 原始错误:", insertErr);
      if (insertErr?.code === "42P01") {
        return NextResponse.json(
          { error: "保存失败：数据库里还没有这张表。需要在 Supabase 后台的 SQL Editor 里跑一次 SQL-v228-romance-romance-tables.sql。" },
          { status: 500 }
        );
      }
    ...
    console.error("[romance save] 计算失败:", e);
