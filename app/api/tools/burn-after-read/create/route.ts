...
export async function POST(req:NextRequest){
 ...
 const admin=createAdminClient();const ip=(req.headers.get("x-forwarded-for")||"unknown").split(",")[0].trim();const rl=await admin.rpc("rate_limit_check",{p_key:`burn-create:${ip}`,p_limit:40,p_window_seconds:3600});if(rl.error)return NextResponse.json({error:"RATE_GUARD_UNAVAILABLE"},{status:503});if(rl.data!==true)return NextResponse.json({error:"RATE_LIMITED"},{status:429});
 const expiresAt=new Date(Date.now()+ttl*60000).toISOString();const{data,error}=await admin.from("burn_notes").insert({ciphertext,iv,expires_at:expiresAt,mode,max_views:maxViews,view_duration_seconds:mode==="fast"?duration:null}).select("id,expires_at").single();
 if(error||!data)return NextResponse.json({error:"NOTE_CREATE_FAILED"},{status:500});return NextResponse.json({id:data.id,expiresAt:data.expires_at});
}
