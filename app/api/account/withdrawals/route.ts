...
export async function GET(){
  ...
  const [ordersResult,withdrawalsResult]=await Promise.all([
    ...
    admin.from("balance_withdrawals")
      .select("id,order_id,wallet_kind,provider,currency,provider_currency,amount_minor,provider_amount_minor,status,provider_status,failure_code,created_at,completed_at")
      .eq("user_id",user.id).order("created_at",{ascending:false}).limit(100),
  ]);
  if(ordersResult.error||withdrawalsResult.error)return NextResponse.json({error:"WITHDRAWAL_DATA_FAILED"},{status:500});
...
export async function POST(req:NextRequest){
  ...
  const withdrawalId=d.withdrawalId;
  const marked=await admin.from("balance_withdrawals")
    ...
    .select("id").single();
  if(marked.error){
    await admin.rpc("release_balance_withdrawal",{p_withdrawal_id:withdrawalId,p_failure_code:"PROCESSING_STATE_FAILED",p_provider_status:null});
    return NextResponse.json({error:"WITHDRAWAL_STATE_FAILED"},{status:500});
  }

  if(!order.provider_payment_id){
    await admin.rpc("release_balance_withdrawal",{p_withdrawal_id:withdrawalId,p_failure_code:"PROVIDER_PAYMENT_ID_MISSING",p_provider_status:null});
    return NextResponse.json({error:"ORIGINAL_PAYMENT_REFERENCE_MISSING"},{status:409});
  }

  const providerOrderTotal=d.providerCurrency==="USD"
    ?Math.round(Number(order.amount_usd||0)*100)
    :Math.round(Number(order.amount_rmb||0)*100);
  ...
  try{
    const attempt=await executeProviderRefund({
      ...
      withdrawalId,
      currency:d.providerCurrency,
      orderAmountMinor:providerOrderTotal,
      refundAmountMinor:Number(d.providerAmountMinor),
    });

    if(attempt.state==="completed"){
      const done=await admin.rpc("complete_balance_withdrawal",{p_withdrawal_id:withdrawalId,p_provider_refund_id:attempt.refundId||"",p_provider_status:attempt.providerStatus});
      if(done.error||!(done.data as {ok?:boolean}|null)?.ok)return NextResponse.json({ok:true,status:"processing",withdrawalId},{status:202});
      return NextResponse.json({ok:true,status:"completed",withdrawalId});
    }
    if(attempt.state==="failed"){
      await admin.rpc("release_balance_withdrawal",{p_withdrawal_id:withdrawalId,p_failure_code:"PROVIDER_REFUND_REJECTED",p_provider_status:attempt.providerStatus});
      return NextResponse.json({error:"PROVIDER_REFUND_REJECTED",withdrawalId},{status:409});
    }

    await admin.from("balance_withdrawals").update({provider_refund_id:attempt.refundId,provider_status:attempt.providerStatus,updated_at:new Date().toISOString()}).eq("id",withdrawalId);
