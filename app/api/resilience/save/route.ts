...
// v228：生命韧性指数从"免费单页快测"升级出一条付费深度报告的产品线，
// 保存流程照抄 qian_submissions 那一套已经验证过的写法（登录校验、
// admin client写库、错误信息里带上原始Postgres错误方便排查）——不是
// 我重新设计一套新写法，是把已经证明安全、好用的模式原样复制过来。
export async function POST(req: Request) {
  ...
  try {
    ...
    if (insertErr || !submission) {
      console.error("[resilience save] 写入失败，Supabase 原始错误:", insertErr);
      if (insertErr?.code === "42P01") {
        return NextResponse.json(
          { error: "保存失败：数据库里还没有这张表。需要在 Supabase 后台的 SQL Editor 里跑一次 SQL-v228-resilience-romance-tables.sql。" },
          { status: 500 }
        );
      }
    ...
    console.error("[resilience save] 计算失败:", e);
