...
export const NARRATIVES: Narrative[] = [
  ...
  { slug: "shipyard-chronicles-i", title: "船坞纪 · 一艘不愿意战斗的舰", titleEn: "Shipyard Chronicles I · The Ship That Refused to Fight", cat: "novel", teaser: "在忆核船坞，一艘舰的心不是造出来的，是一个活人愿意亲手交出的记忆，跟星屑融合长出来的。年轻锻造者阮惊蛰淬出的那颗心，怎么都不肯要炮塔，只想带人回家。", teaserEn: "At the Memory-Root Shipyard, a ship's heart isn't built — it's grown from a memory someone chooses to give away. The heart Ruan Jingzhe quenches refuses every weapon interface. It only wants to carry people home.", price: 5, cover: `<svg viewBox="0 0 300 180" xmlns="http://www.w3.org/2000/svg"><defs><radialGradient id="syc-bg" cx="50%" cy="42%" r="70%"><stop offset="0%" stop-color="#3a2350"/><stop offset="55%" stop-color="#1c1330"/><stop offset="100%" stop-color="#05030a"/></radialGradient><filter id="syc-blur"><feGaussianBlur stdDeviation="8"/></filter></defs><rect width="300" height="180" fill="url(#syc-bg)"/><g opacity=".5" filter="url(#syc-blur)"><ellipse cx="150" cy="110" rx="120" ry="34" fill="#5c3560"/><circle cx="90" cy="70" r="24" fill="#e8845f"/></g><path d="M70 118 Q150 90 230 118 L215 132 Q150 112 85 132 Z" fill="#2a1c3a" stroke="#e8b28f" stroke-width="1" opacity=".8"/><circle cx="150" cy="112" r="8" fill="#ffd9a8"><animate attributeName="r" values="6;10;6" dur="3s" repeatCount="indefinite"/><animate attributeName="opacity" values=".7;1;.7" dur="3s" repeatCount="indefinite"/></circle><g fill="#f4ecff" opacity=".6">${Array.from({length:14}).map(()=>{const x=(Math.random()*300).toFixed(0),y=(Math.random()*90).toFixed(0),r=(Math.random()*1.1+.3).toFixed(1);return `<circle cx="${x}" cy="${y}" r="${r}"/>`;}).join('')}</g></svg>` },
  ...
  { slug: "the-grief-weavers", title: "织哀者", titleEn: "The Grief-Weavers", cat: "field", teaser: "他们不修炼快乐，修炼的是怎么把悲伤完整地经历一遍，纺成丝线，织成布，盖在刚刚经历失去的人身上——悲伤从来不是坑，是矿。", teaserEn: "They don't practice happiness. They practice living sorrow all the way through, spinning it into thread, and weaving cloth to lay over the newly grieving. Grief was never a pothole. It's ore.", price: 1, cover: `<svg viewBox="0 0 300 180" xmlns="http://www.w3.org/2000/svg"><defs><radialGradient id="gw-bg" cx="50%" cy="45%" r="70%"><stop offset="0%" stop-color="#3a2438"/><stop offset="60%" stop-color="#20141f"/><stop offset="100%" stop-color="#0a060a"/></radialGradient><filter id="gw-blur"><feGaussianBlur stdDeviation="7"/></filter></defs><rect width="300" height="180" fill="url(#gw-bg)"/><g opacity=".45" filter="url(#gw-blur)"><ellipse cx="150" cy="100" rx="90" ry="40" fill="#8f6a86"/></g><g stroke="#e8c9de" stroke-width=".6" opacity=".55">${Array.from({length:9}).map((_,i)=>`<line x1="${70+i*20}" y1="60" x2="${70+i*20}" y2="150"/>`).join('')}${Array.from({length:7}).map((_,i)=>`<line x1="70" y1="${65+i*13}" x2="230" y2="${65+i*13}"/>`).join('')}</g><circle cx="150" cy="100" r="5" fill="#ffe6f2" opacity=".8"><animate attributeName="opacity" values=".4;.9;.4" dur="4s" repeatCount="indefinite"/></circle></svg>` },  { slug: "watcher-from-the-lyra-field", title: "织女星域来信 · 观测者手记 I", titleEn: "A Letter from the Lyra Field · Observer's Notes I", cat: "sovereign", teaser: "一个从未用过\"个体\"这个词的意识场，第一次靠近地球，看见的不是战争或贫穷，而是几十亿具身体挤在一起、却各自被锁进骨头房间里的巨大孤独——直到他们看见了一道光。", teaserEn: "A field of consciousness that has never needed the word \"individual\" draws close to Earth for the first time — and finds not war or poverty, but a vast loneliness. Then it sees a light.", price: 1, cover: `<svg viewBox="0 0 300 180" xmlns="http://www.w3.org/2000/svg"><defs><radialGradient id="wlf-bg" cx="50%" cy="42%" r="70%"><stop offset="0%" stop-color="#3a2352"/><stop offset="55%" stop-color="#1c1331"/><stop offset="100%" stop-color="#05030c"/></radialGradient><filter id="wlf-blur"><feGaussianBlur stdDeviation="8"/></filter></defs><rect width="300" height="180" fill="url(#wlf-bg)"/><g opacity=".5" filter="url(#wlf-blur)"><circle cx="90" cy="60" r="26" fill="#a68fc9"/><circle cx="215" cy="100" r="30" fill="#7fc9a8"/></g><g fill="#f4ecff" opacity=".75">${Array.from({length:22}).map(()=>{const x=(Math.random()*300).toFixed(0),y=(Math.random()*180).toFixed(0),r=(Math.random()*1.2+.3).toFixed(1),d=(2+Math.random()*3).toFixed(1);return `<circle cx="${x}" cy="${y}" r="${r}"><animate attributeName="opacity" values="0;.9;0" dur="${d}s" repeatCount="indefinite"/></circle>`;}).join('')}</g><path d="M150 150 L150 70" stroke="#f4ecff" stroke-width="1.2" opacity=".6"><animate attributeName="opacity" values=".3;.9;.3" dur="3.6s" repeatCount="indefinite"/></path><circle cx="150" cy="62" r="7" fill="#fff6e8"><animate attributeName="r" values="5;9;5" dur="3.2s" repeatCount="indefinite"/></circle></svg>` },
