...
import { videoPlanCapability } from "./video/capability";
import { utilityOrganizeCapability, utilityStatsCapability } from "./utility/capability";
import { validateResult } from "./validation";

const capabilities:Capability[]=[knowledgeAnswerCapability,knowledgeSummaryCapability,videoPlanCapability,utilityOrganizeCapability,utilityStatsCapability];
...
export async function runAutonomousTask(task:SasiTask,partial?:Partial<CapabilityContext>):Promise<SasiResult>{
 ...
 try{return validateResult(await capability.run(task,ctx));}catch(error){ctx.log("task.failed",{taskId:task.id,error:error instanceof Error?error.message:String(error)});return{ok:false,taskId:task.id,engine:"autonomous",capability:capability.id,artifacts:[],error:{code:"AUTONOMOUS_EXECUTION_FAILED",message:"处理没有完成，请重新尝试。"}};}
}
