import type { CapabilityContext, SasiArtifact, SasiExecutionEvent, SasiResult, SasiTask } from "./types";
...
export type GraphNode = {
  ...
  maxRetries?: number;
  timeoutMs?: number;
  run: (ctx: CapabilityContext, artifacts: ReadonlyMap<string, SasiArtifact[]>) => Promise<SasiArtifact[]>;
...
function ensureGraph(graph: ExecutionGraph) {
  ...
  for (const node of graph.nodes) {
    for (const dep of node.dependsOn ?? []) if (!ids.has(dep)) throw new Error(`GRAPH_NODE_MISSING:${dep}`);
  }
}
...
export async function executeGraph(input: {
  ...
  capability: SasiResult["capability"];
}): Promise<SasiResult> {
  ensureGraph(input.graph);
  ...
  return {
    ...
    artifacts: [...artifacts.values()].flat(),
    history,
  };
}
