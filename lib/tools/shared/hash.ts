/** SHA-256 via Web Crypto (secure contexts only). */
export async function sha256Hex(data: ArrayBuffer): Promise<string> {
  const digest = await crypto.subtle.digest("SHA-256", data);
  return bufferToHex(digest);
}

/** MD5 pure JS — Web Crypto does not provide MD5. */
...
export async function buffersEqual(a: ArrayBuffer, b: ArrayBuffer): Promise<boolean> {
  if (a.byteLength !== b.byteLength) return false;
  // For large files compare hashes first then optional byte check
  if (a.byteLength > 2 * 1024 * 1024) {
