...
const stellarMapPicker = read("app/stellar-trace/PreciseMapPicker.tsx");
const stellarLocationPage = read("miniapp/pages/stellar-location/index.js");
...
check("Mini Program opens Stellar Trace only through the shared web field", /item\.productId === 'stellar-trace'[\s\S]{0,180}pages\/web\/index/.test(exploreClient) && /options\.product === 'stellar-trace'[\s\S]{0,180}wx\.redirectTo/.test(productClient) && /productId === "stellar-trace"\) return "\/stellar-trace"/.test(contentDestinations));
check("Mini Program Stellar Trace uses native WeChat map selection and returns coordinates to the web field", app.pages.includes("pages/stellar-location/index") && /wx\.chooseLocation/.test(stellarLocationPage) && /pickedLat/.test(stellarLocationPage) && /miniProgram\?\.navigateTo/.test(stellarMapPicker) && /pickedLabel/.test(stellarExperience));
...
check("Stellar Trace web field owns the complete intake before payment", ["寻踪对象姓名", "与寻踪对象的关系", "真实出生日期", "出生时间", "出生地点", "最后有效联系日期", "最后有效联系时间", "最后已知位置说明", "PreciseMapPicker", "最后一次已知移动方向", "最后一次有效信息"].every((term) => stellarExperience.includes(term)) && /STELLAR_TRACE_DRAFT_KEY/.test(checkout) && /确认边界并开启/.test(stellarExperience));
check("Stellar Trace native form is unreachable even through an old deep link", productClient.indexOf("options.product === 'stellar-trace'") < productClient.indexOf("this.options = options") && /wx\.redirectTo\(\{ url: `\/pages\/web\/index/.test(productClient));
check("Stellar Trace accepts historical subjects from year 0001 through today", /min="0001-01-01"/.test(stellarExperience));
...
check("Stellar Trace discloses non-convergence before payment", /支付前结果边界/.test(stellarExperience) && /现实移动方向仅用于事后核验/.test(stellarExperience) && /不把古法里数伪换算为公里或坐标/.test(stellarExperience));
check("Web Stellar Trace hand-off keeps intake off the URL", /window\.name=JSON\.stringify\(\{kind:"lingxifield-stellar-trace-draft-v3"/.test(stellarExperience) && !/searchParams\.set\(["']stellarDraft/.test(stellarExperience));
check("report discovery keeps nine dendrite assessments and one Stellar Trace web field", /pages\/assessment\/index/.test(exploreClient) && /item\.productId === 'stellar-trace'[\s\S]{0,180}pages\/web\/index/.test(exploreClient));
...
check("native Stellar Trace preserves person, object, animal and subtype through the secure hand-off", /targetSubtypeIndex/.test(productClient) && /objectSubtypeValues/.test(productClient) && /animalSubtypeValues/.test(productClient) && /targetSubtype:this\.data/.test(productClient) && /targetSubtype/.test(stellarMiniIntake));
