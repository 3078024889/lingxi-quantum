...
const stellarRetiredPage = read("app/stellar-trace/page.tsx");
const stellarApi = read("app/api/stellar-trace/route.ts");
const stellarTargetApi = read("app/api/stellar-trace/target/route.ts");
...
check("Stellar Trace is absent from every active catalog and public discovery surface", !/id: "stellar-trace"/.test(plans) && !/rpt_stellar_trace/.test(catalog) && !/field: "00"/.test(fieldProductCopy) && !/href:"\/stellar-trace"/.test(fieldInsights));
check("Stellar Trace old deep links resolve only to the retirement notice", /options\.product === 'stellar-trace'[\s\S]{0,180}wx\.redirectTo/.test(productClient) && /星迹已停止开放/.test(stellarRetiredPage) && /历史订单与档案记录会继续保留/.test(stellarRetiredPage));
check("Stellar Trace no longer requests Mini Program location permission or ships a native location page", !app.pages.includes("pages/stellar-location/index") && !app.permission && !app.requiredPrivateInfos && !fs.existsSync(path.join(root, "miniapp/pages/stellar-location/index.js")));
check("Stellar Trace generation and all payment entry points are hard-blocked", [stellarApi, stellarTargetApi, payCreate, webPayCreate, wechatPayCreate].every((source) => /status:\s*410/.test(source)));
check("Stellar Trace historical entitlements retain a safe route to the retirement notice", /"stellar-trace"/.test(contentDestinations) && /body\.productId !== "stellar-trace"/.test(contentLink));
check("report discovery keeps only the active dendrite assessments", /pages\/assessment\/index/.test(exploreClient) && /productId !== 'stellar-trace'/.test(exploreClient) && !/¥688/.test(exploreView));
