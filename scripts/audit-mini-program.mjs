...
check("Stellar Trace Mini Program uses one canonical v3 intake with legacy-cache removal and IME-safe delayed persistence", /lingxifield_stellar_trace_draft_v3/.test(stellarMiniIntake) && /draft_v2/.test(stellarMiniIntake) && /setTimeout\(\(\) => this\.persistStellarDraft\(\), 280\)/.test(productClient) && /return value/.test(productClient) && /sanitizeStellarDraft/.test(productClient) && /buildStellarDraft\(\)/.test(productClient) && !/stellarDraft:\s*\{/.test(productClient) && !/stellarDraft\./.test(productView));
