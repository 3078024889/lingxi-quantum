...
check("Life Archetype is a versioned automatic eight-field convergence", /layer: "convergence"/.test(fieldProductCopy) && /calculateLifeArchetypeFromReports/.test(dendriteEngine) && /BASE_DENDRITE_PRODUCT_IDS/.test(dendriteEngine) && /lingxifield-mini-archetype-v3/.test(`${dendriteEngine}\n${lifeArchetype}`));
...
check("Life Archetype owns a dedicated 22-page dendritic publication", /index=\{22\}/.test(miniLifeArchetypeReport) && /Eight-stream Convergence Map/.test(miniLifeArchetypeReport) && /八流汇聚/.test(miniLifeArchetypeReport));
check("all ten assessments generate 24 evidence-linked report entries", (reportEntryLibrary.match(/blueprint\(/g) || []).length === 10 && /titlesZh\.map/.test(reportEntryLibrary) && /reportEntries/.test(dendriteEngine));
