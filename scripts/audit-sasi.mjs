...
const requiredFiles = [
  "lib/sasi/autonomy/types.ts",
  "lib/sasi/autonomy/capability-graph.ts",
  "lib/sasi/autonomy/router.ts",
  "lib/sasi/autonomy/resource-governor.ts",
  "lib/sasi/autonomy/provenance.ts",
  "lib/sasi/autonomy/refresh-planner.ts",
  "lib/sasi/autonomy/promotion-gate.ts",
...
// Autonomous core
const router = read("lib/sasi/autonomy/router.ts");
...
const graph = read("lib/sasi/autonomy/capability-graph.ts");
for (const id of [
  "document-parse",
  "file-convert",
  "knowledge-retrieve",
...
must(ask.includes('routeSasiTask("knowledge-answer")'), "ASK_ROUTER_NOT_WIRED");
...
for (const rel of [
  ...
  must(!/\b(RPC|Pipeline|Worker|Queue|Inference|Endpoint|Webhook|Object Storage|Runtime)\b/i.test(text),
...
console.log("SASI_AUTONOMY_CORE=PASS");
