...
const checks = [
  ...
  ["account center avoids fabricated commerce and audit data", panels.includes("统一下载审计尚未开放") && panels.includes("FROM IDEA TO CREATION") && panels.includes("先看方案与预算，再决定开始")],
  ["manifestation exposes six distinct visual chapters", ["六段显化路径","从安静下来，到把愿景写进今天","mf-path-grid","mf-reader","mf-rail","IntersectionObserver","今日执行卡"].every((value)=>`${manifestation}\n${manifestationGuide}`.includes(value)) && Array.from({length:6},(_,i)=>`/images/manifestation/chapter-0${i+1}.png`).every((value)=>manifestationGuide.includes(value) && fs.existsSync(`public${value}`))],
  ["manifestation gates full guidance behind real access", manifestation.includes("!manifestActive ?") && manifestation.includes("ManifestationChapters") && manifestation.includes("RealityLoop") && manifestation.includes("mf-daily-panel")],
  ["manifestation preserves real records and save failures", ["visions","reality_entries","今日记录没有保存成功","new Set","entry_date"].every((value)=>realityLoop.includes(value))],
  ...
  ["life map keeps real preview payment and in-workspace archive boundaries", ["/api/lifemap/calc","/api/lifemap/save","/checkout?productId=life-map-report","initialArchiveId","FullReportView","生成免费预览","解锁完整档案"].every((value)=>lifeMapFlow.includes(value)) && !lifeMapFlow.includes("¥199") && !lifeMapFlow.includes("不是你的已生成 PDF")],
  ["field entrances use distinct actions and artwork", ["生成我的生命图谱","开启我的生命灵签","生成我的三重生命镜像","选择关系并生成共振图谱","生成我的财富创造地图","读取我的今日潮汐","生成我的生命韧性指数","连接我的桃花磁场","查看汇流进度"].every((value)=>fieldTests.includes(value)) && fieldTests.includes("/images/lifemap-types/yongliuzhe.jpg")],
