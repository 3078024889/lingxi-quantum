...
for(const token of ['const stages = ["定时", "落证", "合度", "显域"]','data-stellar-visualization={mode}','mode === "print"','候选坐标 · <strong>尚未成域</strong>'])if(!visualization.includes(token))failures.push(`visualization missing ${token}`);
...
for(const token of ['循时而索迹，因星而见位。','title="四证合度"','title="九域据链 · 推演边界"','权益自支付成功起 7 天内有效','支付前结果边界','不保证形成唯一候选坐标','模型停止于证界不等同于技术故障'])if(!experience.includes(token))failures.push(`experience missing ${token}`);
