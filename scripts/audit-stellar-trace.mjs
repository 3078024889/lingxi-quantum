...
for(const token of ['version:"lingxifield-stellar-trace-v3"','priorityFrom(direction,evidence,input)','candidateRegions:[]','candidateCenter:null','rangeKm:null','resultingRangeKm:null'])if(!engine.includes(token))failures.push(`engine missing ${token}`);
...
for(const token of ['循时而索迹，因星而见位。','title="诸证合度"','title="九域据链 · 现实核验"','主核验方位','现实核验次序','权益自支付成功起 7 天内有效','支付前结果边界','不保证形成唯一候选坐标','模型停止于证界不等同于技术故障'])if(!experience.includes(token))failures.push(`experience missing ${token}`);
