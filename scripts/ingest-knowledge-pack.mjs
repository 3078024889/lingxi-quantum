...
const readme = existsSync(join(packRoot, "00-README.md")) ? readFileSync(join(packRoot, "00-README.md"), "utf8") : slug;
...
if (existing.data) { sourceId = existing.data.id; console.log("[source] dedup", sourceId); }
else {
  const { data, error } = await admin.from("cangxuan_sources").insert({
    user_id: userId, title: String(title).slice(0,160), source_type: "teacher_synthetic",
    rights_scope: "opted_in_training", trainability: "trainable",
    license_metadata: { pack: slug, kind: "self-knowledge" },
  ...
  sourceId = data.id; console.log("[source]", sourceId);
}
let ok=0;
for (const item of knowledge) {
  const { error } = await admin.from("cangxuan_knowledge_items").upsert({
    user_id: userId, source_id: sourceId, category: item.category, title: item.title,
    statement: String(item.statement).slice(0,1000), evidence: String(item.evidence).slice(0,1200),
    tier: item.tier || "gold", trainability: "trainable", quality_score: 0.93,
