...
for (const [b,n] of thins){
  ...
  const tag = st.size>=2000 && bl>=2000 ? 'READABLE' : (bl>=2000?'READABLE_BODY': (st.size>=2000?'BYTES_OK_BODY_THIN':'THIN'));
  console.log(`${tag}\tbytes=${st.size}\tbody=${bl}\t${b}/${n}`);
}
console.log('pdf candidates:', fs.existsSync('_tools/_thin-refetch/pdf-candidates.txt')?fs.readFileSync('_tools/_thin-refetch/pdf-candidates.txt','utf8'):'none');
